...
- Setup/adoption/update = follow [integration setup](../he/references/integrations.md) for intended services, hosting, existing choices and real host readiness; greenfield imports alone cannot identify future integrations.
...
- Delivery = keep build acceptance local; retain the full authorized remote destination + required proof as pending delivery prose in Verification for [HE Ship](../he-ship/SKILL.md). Local Complete precedes shipping, never falsely completes it. For UI work, retain the actual before capture for the later PR comparison; prepare task isolation before implementation.
...
- Draft handoff = questions may end a turn while planning is incomplete; explicitly retain missing UX/baseline proof and the unresolved decisions. Acceptance of an early decision pack does not complete the later plan/UX readiness check. Stop reports incomplete Draft evidence and may request bounded continuation; its feedback grants no implementation or broader scope authority.
