...
- Cleanup = only after confirmed merge and required delivery proof. From a persistent checkout, deliberately clean only known generated artifacts, then run the guard; preserve current/main, dirty/untracked/unknown ignored, locked, reused or changed task worktrees/branches. Do not delete another active task's checkout. Uncertain ownership/activity → retain it and report the blocker. No force-removal to make cleanup pass.
