...
| Marionette | Optional debug-only integration. Registration trigger = a `marionette_flutter` dependency, pinned to the `pubspec.lock` version; detection runs only on install or an applied update, so a dependency added between updates or a later lock upgrade needs a manual entry edit. Registration alone does not configure or connect a running app; [Flutter recorded proof](../../e2e/references/flutter.md) owns the binding, connection and recording steps. |
