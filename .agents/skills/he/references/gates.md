...
The check validates structure + declarations, not evidence truth, approval or relevance. A passing Draft/Ready run is not implementation completion. A starting-baseline Exception does not waive current native checks. If the Ready or Complete command fails, return the same plan to Draft with the actual blocker; do not replace failure with N/A or leave a false Ready/Complete claim. Resolve the cause through authorized repair or a justified native exception at the existing check owner, then rerun.
