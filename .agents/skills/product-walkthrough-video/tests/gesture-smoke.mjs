...
try {
	...
	preflightError(
		"playwright is not installed; run pnpm install --frozen-lockfile --ignore-scripts from this package directory",
	);
}
if (!existsSync(chromiumExecutable)) {
	preflightError(
		"Chromium is not installed; run pnpm exec playwright install chromium from this package directory",
	);
}
...
const cleanReviewStatuses = slowCapture
...
function unexpectedFindings(report) {
	return report.findings.filter(
		(finding) => !(slowCapture && captureSpeedKinds.has(finding.kind)),
	);
}
...
try {
	...
	const unsafeConfigPath = path.join(
		workspace,
		"unsafe-walkthrough.config.json",
	);
	const invalidStepConfigPath = path.join(
		workspace,
		"invalid-step-walkthrough.config.json",
	);
	...
	const noOpReportPath = path.join(
		outputDir,
		"gesture-smoke-no-op-run-report.json",
	);
	const noOpReviewPath = path.join(
		outputDir,
		"gesture-smoke-no-op-review.json",
	);
	const ambiguousReportPath = path.join(
		outputDir,
		"gesture-smoke-ambiguous-run-report.json",
	);
	const ambiguousReviewPath = path.join(
		outputDir,
		"gesture-smoke-ambiguous-review.json",
	);
	const config = {
		name: "gesture-smoke",
	...
	const typeKeysIndex = invalidStepConfig.steps.findIndex(
		(step) => step.action === "typeKeys",
	);
	const pressIndex = invalidStepConfig.steps.findIndex(
		(step) => step.action === "press",
	);
	delete invalidStepConfig.steps[typeKeysIndex].text;
	delete invalidStepConfig.steps[pressIndex].target;
	await writeFile(
		invalidStepConfigPath,
		`${JSON.stringify(invalidStepConfig, null, 2)}\n`,
		"utf8",
	);
	...
	assert(
		invalidStepRun.code === 1,
		`Invalid strict steps exited ${invalidStepRun.code}`,
	);
	assert(
		invalidStepOutput.includes(
			"preserveVisualDuringReload is only valid for reload steps",
		),
	...
	assert(
		report.skill?.version === "2.8.0",
		`Unexpected skill version ${report.skill?.version}`,
	);
	assert(
		report.timeline.some(
			(step) =>
				step.action === "typeKeys" && step.metadata?.charactersTyped === 3,
		),
	...
	assert(
		report.timeline.some(
			(step) =>
				step.action === "reload" &&
				step.metadata?.visualContinuityGuard?.kind ===
					"document-start-snapshot-bridge",
		),
	...
	assert(
		report.findings.some(
			(finding) =>
				finding.kind === "expected-http-response" &&
	...
	assert(
		report.pointerTrack.some(
			(event) => event.kind === "move" && event.pressed === true,
		),
	...
	assert(
		keyboardPress?.metadata?.inputEvents?.[0]?.inputType === "keyboard" &&
			...
			keyboardPress.metadata.inputEvents[0].pointerVisibleAtActivation ===
				false &&
	...
	assert(
		report.pointerTrack.some(
			(event) => event.kind === "visibility" && event.visible === false,
		) &&
			report.pointerTrack.some(
				(event) => event.kind === "visibility" && event.visible === true,
			),
	...
	assert(
		reviewReport.gestureAudits?.length === 1,
		"Gesture audit was not generated",
	);
	if (slowCapture) {
		assert(
		...
		assert(
			reviewReport.gestureAudits[0].smooth === true,
			"Gesture audit did not pass",
		);
	}
	assert(
		reviewReport.navigationAudits?.length === 1,
		"Reload navigation audit was not generated",
	);
	assert(
	...
	const ambiguousPress = ambiguousReport.timeline.find(
		(step) => step.action === "press",
	);
	ambiguousPress.metadata.inputEvents[0].pointerVisibleAtActivation = true;
	ambiguousPress.metadata.inputEvents[0].pointerPositionAtActivation = {
		x: 30,
		y: 30,
	};
	await writeFile(
		ambiguousReportPath,
		`${JSON.stringify(ambiguousReport, null, 2)}\n`,
		"utf8",
	);
	...
	const ambiguousReviewReport = JSON.parse(
		await readFile(ambiguousReviewPath, "utf8"),
	);
	assert(
	...
	await writeFile(
		noOpReportPath,
		`${JSON.stringify(noOpReport, null, 2)}\n`,
		"utf8",
	);
	...
	assert(
		noOpReviewReport.findings.some(
			(finding) => finding.kind === "scroll-no-op",
		),
	...
	const proofReportPath = path.join(
		proofDir,
		"gesture-smoke-proof-run-report.json",
	);
	const proofReviewPath = path.join(
		proofDir,
		"gesture-smoke-proof-review.json",
	);
	const proofConfig = {
		...config,
	...
	const proofRun = await runNode(runner, [
		"--config",
		proofConfigPath,
		"--output-dir",
		proofDir,
