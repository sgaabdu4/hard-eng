...
try {
  ...
  `);
	...
	await page.evaluate(
		() => Reflect.get(window, "__walkthroughPointerTransition")?.finished,
	);
	const minimum = Math.min(...counts);
	if (minimum < 50) {
		throw new Error(
			`pointer disappeared during the document view transition: ${counts.join(",")}`,
		);
	}
	console.log(
		`view-transition-pointer-smoke: PASS min=${minimum} samples=${counts.join(",")}`,
	);
} finally {
