	...
	return new Promise((resolve, reject) => {
		...
		child.on("close", (code) => {
			...
			if (code !== 0)
				reject(
					new Error(
						result.stderr.trim() || `${commandName} exited with ${code}`,
					),
				);
			else resolve(result);
		});
	...
	for (const event of pointerAudit.track) {
		...
		if (timeMs <= event.endMs) {
			...
			const linearProgress = Math.max(
				0,
				Math.min(1, (timeMs - event.startMs) / duration),
			);
...
function distanceToSegment(x, y, segment) {
	...
	if (lengthSquared === 0)
		return Math.hypot(x - segment.fromX, y - segment.fromY);
	const progress = Math.max(
		0,
		Math.min(
			1,
			((x - segment.fromX) * deltaX + (y - segment.fromY) * deltaY) /
				lengthSquared,
		),
	);
	return Math.hypot(
...
function findRingCandidate(
	...
	for (let index = 0; index < pixelCount; index += 1) {
		...
		if (count < 6 || width < minimumDiameter || height < minimumDiameter)
			continue;
		if (
		...
		for (const point of points) {
			const quadrant =
				(point.y >= centerY ? 2 : 0) + (point.x >= centerX ? 1 : 0);
			quadrants[quadrant] += 1;
			if (Math.abs(point.x - centerX) <= 1 && Math.abs(point.y - centerY) <= 1)
				centerPixels += 1;
		}
		...
		const pathDistance = movementSegment
			...
			: distance;
		const allowedPathDistance = movementSegment
			? Math.max(18, renderedSize * 4)
			: maximumDistance;
		if (pathDistance > allowedPathDistance) continue;
		const score = movementSegment
			? pathDistance * 4 + distance * 0.05
			: distance;
...
function inspectFrame(
	...
	for (let offset = 0; offset < frame.length; offset += 3) {
		...
		if (previous) {
			const prior =
				(previous[offset] * 77 +
					previous[offset + 1] * 150 +
					previous[offset + 2] * 29) >>
				8;
	...
	if (pointerAudit?.enabled && pointerAudit.color) {
		...
		const baseRadius = Math.max(
			6,
			pointerAudit.size * Math.max(scaleX, scaleY) * 1.4,
		);
		...
		if (movementSegment) {
			...
			const corridorLeft = Math.max(
				0,
				Math.floor(
					Math.min(movementSegment.fromX, movementSegment.toX) - corridorRadius,
				),
			);
			const corridorRight = Math.min(
				scanWidth - 1,
				Math.ceil(
					Math.max(movementSegment.fromX, movementSegment.toX) + corridorRadius,
				),
			);
			const corridorTop = Math.max(
				0,
				Math.floor(
					Math.min(movementSegment.fromY, movementSegment.toY) - corridorRadius,
				),
			);
			const corridorBottom = Math.min(
				scanHeight - 1,
				Math.ceil(
					Math.max(movementSegment.fromY, movementSegment.toY) + corridorRadius,
				),
			);
			for (let y = corridorTop; y <= corridorBottom; y += 1) {
				for (let x = corridorLeft; x <= corridorRight; x += 1) {
					if (distanceToSegment(x, y, movementSegment) > corridorRadius)
						continue;
					const offset = (y * scanWidth + x) * 3;
					if (
						pointerColorMatches(
...
async function scanFrames(videoPath, probe, pointerAudit) {
	...
	const scanHeight = Math.max(
		1,
		Math.round((sourceHeight / sourceWidth) * scanWidth),
	);
	...
	const inspect = (frame) => {
		...
		if (diff !== null) {
			diffSeries.push({
				...
				blank: metrics.blank,
				pointerPresent: metrics.pointer?.expectedVisible
					? metrics.pointer.present
					: null,
				unexpectedPointer:
					metrics.pointer?.expectedVisible === false
						? metrics.pointer.present
						: false,
			});
		...
		const pointerMotion = Boolean(
			metrics.pointer?.moving && metrics.pointer.present,
		);
		...
		if (metrics.pointer?.expectedVisible && !metrics.pointer.present) {
			...
			pointerMissingRuns.push({
				startFrame: pointerMissingStart,
				endFrame: frameIndex - 1,
			});
		...
		if (
			metrics.pointer &&
			!metrics.pointer.expectedVisible &&
			metrics.pointer.present
		) {
	...
	if (pending.length !== 0)
		throw new Error(
			`Video ended with an incomplete frame (${pending.length} bytes)`,
		);
	if (blankKind)
		blankRuns.push({
			kind: blankKind,
	...
	if (staticActive)
		staticRuns.push({ startFrame: staticStart, endFrame: frameIndex });
	if (pointerMissingActive)
		pointerMissingRuns.push({
			startFrame: pointerMissingStart,
			endFrame: frameIndex,
...
async function createActionSheets(videoPath, outputDir, timeline) {
	...
	for (const step of timeline) {
		...
		const actionEndMs = Math.max(
			startMs,
			finiteNumber(step.actionEndMs, startMs),
		);
		const stepEndMs = Math.max(
			actionEndMs,
			finiteNumber(step.endMs, actionEndMs),
		);
