...
async function run() {
	...
	if (!args.config || args.config === true)
		throw new Error(
			"Usage: run-walkthrough.mjs --config /path/walkthrough.config.json",
		);
	...
	if (configProblems.length > 0)
		throw new Error(
			`Walkthrough config failed preflight:\n- ${configProblems.join("\n- ")}`,
		);
	...
	try {
		...
		throw new Error(
			`Refusing to overwrite an existing attempt: ${plannedVideoPath}`,
		);
	} catch (error) {
	...
	if (config.captureStepScreenshots !== false)
	...
	const pointerSource =
		config.pointer === false ? { enabled: false } : config.pointer || {};
	...
	try {
		context = await browser.newContext({
			...
			storageState: config.storageState
				? path.resolve(configDir, config.storageState)
				: undefined,
		});
		...
		await context.route("**/*", async (route) => {
			...
			if (/^https?:/i.test(url)) {
				...
				const sensitiveInRequest = findSensitive(
					`${url}\n${request.postData() || ""}`,
				);
				if (sensitiveInRequest.length > 0)
					recordSensitive("request metadata", sensitiveInRequest);
		...
		page.on("console", (message) => {
			...
			const allowed = includesAllowedSubstring(
				text,
				config.allowedConsoleMessageSubstrings,
			);
		...
		page.on("pageerror", (error) => {
			const allowed = includesAllowedSubstring(
				error.message,
				config.allowedPageErrorSubstrings,
			);
		...
		page.on("requestfailed", (request) => {
			...
			const detail =
				`${request.method()} ${safeUrl(request.url())} ${failure}`.trim();
			const allowed = includesAllowedSubstring(
				request.url(),
		...
		await page.goto(resolveUrl(baseUrl, firstStep.url || "/"), {
			waitUntil:
				firstStep.navigationWaitUntil ||
				config.navigationWaitUntil ||
				"domcontentloaded",
			timeout: finiteNumber(
				firstStep.timeoutMs ?? config.readyTimeoutMs,
				30000,
				1,
			),
		});
		...
		if (firstCheckpoint)
			lastCheckpointPath = path.join(checkpointDir, firstCheckpoint);
		timeline.push({
			index: 1,
		...
		for (let index = 1; index < config.steps.length; index += 1) {
			...
			try {
				...
				const holdMs =
					step.action === "pause"
						? 0
						: finiteNumber(step.holdMs ?? config.stepHoldMs, 1200);
				if (holdMs > 0) await page.waitForTimeout(holdMs);
				const checkpoint = await captureCheckpoint(index, step);
				if (checkpoint)
					lastCheckpointPath = path.join(checkpointDir, checkpoint);
				const endMs = clock();
				timeline.push({
					index: index + 1,
					label: step.label,
