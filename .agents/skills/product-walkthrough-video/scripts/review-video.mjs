...
function assertPlaybackEvidence(
	evidence,
	expectedVideo,
	...
	if (evidence?.status !== "passed" || evidence.schemaVersion !== 2) {
		throw new Error(
			`${label} is missing passed schema-version-2 playback evidence`,
		);
	}
	...
	if (
		...
		throw new Error(
			`${label} is not bound to the reviewed video's hash, dimensions, and duration`,
		);
	}
	...
	if (
		...
		throw new Error(
			`${label} does not prove uninterrupted, no-seek playback at 1x through ended`,
		);
	}
}
...
function analyzeOpening(diffSeries, openingStableMs, fps) {
	const samples = diffSeries.filter(
		(sample) => sample.timeMs <= openingStableMs,
	);
	const unstable = samples.filter((sample) => sample.diff >= 8);
	const minimumComparedFrames = Math.max(
		1,
		Math.floor((openingStableMs / 1000) * fps * 0.9),
	);
	return {
		...
		maxDiff: samples.reduce(
			(maximum, sample) => Math.max(maximum, sample.diff),
			0,
		),
...
function analyzeScroll(step, diffSeries) {
	const samples = diffSeries.filter(
		(sample) =>
			sample.timeMs >= step.startMs && sample.timeMs <= step.actionEndMs,
	);
	const active = samples.filter((sample) => sample.diff > 1.5);
	const maximum = samples.reduce(
		(value, sample) => Math.max(value, sample.diff),
		0,
	);
	const noOp =
...
function analyzeDrag(step, diffSeries, pointerTrack, pointerMissingFailMs) {
	...
	const pointerMissing = samples.filter(
		(sample) => sample.pointerPresent === false,
	);
	...
	const trackedDistancePx = pressedMoves.reduce(
		(total, event) =>
			total + Math.hypot(event.to.x - event.from.x, event.to.y - event.from.y),
		0,
	);
	...
	const sampleIntervalMs =
		samples.length > 1
			? (samples.at(-1).timeMs - samples[0].timeMs) / (samples.length - 1)
			: 40;
	...
	return {
		...
		trackedDurationMs,
		trackedDistancePx,
		pointerMissingFrames: pointerMissing.map(({ frame, timeMs }) => ({
			frame,
			timeMs,
		})),
...
function analyzeNavigation(step, diffSeries) {
	const navigationStartMs = finiteNumber(
		step.metadata?.navigationStartMs,
		step.startMs,
	);
	const navigationEndMs = finiteNumber(
		step.metadata?.navigationEndMs,
		step.actionEndMs,
	);
	...
	const pointerMissingFrames = samples.filter(
		(sample) => sample.pointerPresent === false,
	);
	...
	return {
		...
		windowEndMs,
		frames: samples.length,
		abruptFrames: abruptFrames.map(({ frame, timeMs, diff }) => ({
			frame,
			timeMs,
			diff,
		})),
		blankFrames: blankFrames.map(({ frame, timeMs, blank }) => ({
			frame,
			timeMs,
			blank,
		})),
		pointerMissingFrames: pointerMissingFrames.map(({ frame, timeMs }) => ({
			frame,
			timeMs,
		})),
		allowedAbruptFrames,
...
async function main() {
	...
	const outputDir = path.resolve(
		String(
			args["output-dir"] || path.join(path.dirname(videoPath), "video-review"),
		),
	);
	const reportPath = path.resolve(
		String(args.report || path.join(outputDir, "video-review.json")),
	);
	...
	const reviewer =
		args.reviewer && args.reviewer !== true ? String(args.reviewer).trim() : "";
	const approvalNotes =
		args.notes && args.notes !== true ? String(args.notes).trim() : "";
	...
	const runReportSha256 = createHash("sha256")
		.update(runReportBuffer)
		.digest("hex");
	...
	if (runReport.error)
		throw new Error(`Run report contains an error: ${runReport.error}`);
	if (runReport.findings?.some((finding) => finding.severity === "error")) {
		throw new Error(
			"Run report contains error findings; refusing video approval",
		);
	}
	...
	const recordedSourcePath = runReport.videoPath
		? path.resolve(runReport.videoPath)
		: null;
	...
	if (!reviewingRecordedSource) {
		...
		if (sourceReview.video?.sha256 !== recordedSourceHash) {
			throw new Error(
				"Approved source review does not match the run report's recorded WebM",
			);
		}
		...
		if (sourceReview.runReportSha256 !== runReportSha256) {
			throw new Error(
				"Approved source review is not bound to the current journey run report",
			);
		}
		if (!sourceReview.runReport)
			throw new Error(
				"Approved source review does not reference its complete run report",
			);
	...
	if (runReport.sensitiveFindings?.length)
		throw new Error(
			"Run report contains sensitive-data findings; refusing video approval",
		);
	if (
		runReport.pointer?.enabled &&
		(!Array.isArray(runReport.pointerTrack) ||
			runReport.pointerTrack.length === 0)
	) {
		throw new Error(
			"Run report enables the pointer but does not contain a pointer trajectory",
		);
	}
	...
	const pointerAudit = runReport?.pointer?.enabled
		? {
				...
				viewport: runReport.recording?.viewport || {
					width: probe.width,
					height: probe.height,
				},
				track: Array.isArray(runReport.pointerTrack)
					? runReport.pointerTrack
					: [],
			}
		: null;
	const { summary: frameScan, diffSeries } = await scanFrames(
		videoPath,
		probe,
		pointerAudit,
	);
	...
	const pointerMissingFailMs = finiteNumber(
		runReport?.thresholds?.pointerMissingFailMs,
		160,
	);
	const openingStableMs = finiteNumber(
		runReport?.thresholds?.openingStableMs,
		800,
	);
	const findings = [];
	const addFinding = (severity, kind, detail) =>
	...
	if (!timestampAudit.monotonic)
		addFinding(
			...
			"Video frame timestamps are not monotonic",
		);
	...
	const expectedDecodedFrames = Math.floor(
		(probe.durationMs / 1000) * probe.fps,
	);
	...
	try {
		...
		openingSheet = await createOpeningSheet(
			videoPath,
			outputDir,
			Math.max(3000, openingStableMs),
		);
	...
	if (
		...
		try {
			playbackEvidence = await auditRealTimePlayback(
				videoPath,
				videoBuffer,
				probe,
			);
	...
	const approvalSucceeded =
		humanApproved && !hasErrors && playbackEvidence?.status === "passed";
	const report = {
		status: hasErrors
			? "failed"
			: approvalSucceeded
		...
		stepCheckpointDirectory: runReport
			? path.join(runReport.outputDir, "step-checkpoints")
			: null,
