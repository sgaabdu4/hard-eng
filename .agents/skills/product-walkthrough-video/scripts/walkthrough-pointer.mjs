...
class WalkthroughPointer {
	...
	async moveTo(x, y, options = {}) {
		...
		if (remainingAnimationMs > 0)
			await this.page.waitForTimeout(remainingAnimationMs);
	...
	async setPressed(pressed) {
		if (!this.options.enabled) return;
		await this.swap(
			pointerHtml(this.options, this.position, this.position, 0, pressed),
		);
		this.track.push({
	...
	async beginClick() {
		...
		await this.swap(
			pointerHtml(this.options, this.position, this.position, 0, true),
		);
	...
	async finishClick(clickCue) {
		...
		if (clickCue.disposePromise) {
			...
			const remaining =
				this.options.rippleMs - (performance.now() - clickCue.startedAt);
