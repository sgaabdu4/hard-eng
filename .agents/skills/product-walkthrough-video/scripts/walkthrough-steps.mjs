...
async function waitForAssets(page, timeoutMs) {
	await page.evaluate(async (assetTimeoutMs) => {
		const timeout = (promise) =>
			Promise.race([
				promise,
				new Promise((resolve) => window.setTimeout(resolve, assetTimeoutMs)),
			]);
...
async function waitForReady(page, config, step = {}) {
	const timeout = finiteNumber(
		step.timeoutMs ?? config.readyTimeoutMs,
		30000,
		1,
	);
	await page.waitForLoadState(
		step.navigationWaitUntil ||
...
async function waitForBoxStability(locator, timeoutMs = 3000) {
	...
	while (performance.now() - startedAt < timeoutMs) {
		...
		if (
			previous &&
			Object.keys(rounded).every((key) => rounded[key] === previous[key])
		)
			stableSamples += 1;
...
async function stableTargetPoint(locator, page, config, spec, label) {
	...
	const box = await waitForBoxStability(
		locator,
		finiteNumber(config.actionTimeoutMs, 30000, 1),
	);
...
async function moveTo(locator, page, config, pointer, spec) {
	const point = await stableTargetPoint(
		locator,
		page,
		config,
		spec,
...
async function preparePointerActivation(
	...
	const stablePoint = pointWithinBox(
		stableBox,
		position,
		"Pointer activation target",
	);
...
async function smoothWheel(page, step, config) {
	...
	const durationMs = finiteNumber(
		step.durationMs ?? config.scrollDurationMs,
		900,
		0,
	);
...
async function runStep(
	...
	const timeout = finiteNumber(
		step.timeoutMs ?? config.actionTimeoutMs,
		30000,
		1,
	);
	...
	if (action === "pause") {
		await page.waitForTimeout(
			finiteNumber(step.ms ?? step.holdMs ?? config.stepHoldMs, 1200),
		);
		return {};
	}
	if (action === "reload") {
		...
		try {
			if (step.preserveVisualDuringReload === true) {
				...
				await page.waitForTimeout(
					finiteNumber(step.visualGuardLeadMs, 500, 250, 2000),
				);
			}
			...
			if (cover) {
				await page.waitForTimeout(
					finiteNumber(step.visualGuardSettleMs, 160, 0, 2000),
				);
				await page.evaluate(
					({ hostId, storageKey }) => {
	...
	if (action === "waitForSelector") {
		await getTarget(page, step).waitFor({
	...
	if (action === "assertText") {
		...
		if (!actual.includes(String(step.text))) {
			throw new Error(
				`Expected visible text was not found: ${redactedMessage(step.text)}`,
			);
		}
		return {};
	}
	...
	if (action === "drag") {
		...
		if (destinationLocator !== sourceLocator)
			await revealLocator(destinationLocator, page, config);
		...
		const source = {
			box: sourceBox,
			...pointWithinBox(sourceBox, step.from, "Drag start"),
		};
		const destination = {
			box: destinationBox,
		...
		if (distancePx < 4)
			throw new Error(
				`Step "${step.label || index}" drag distance is too small`,
			);
		...
		try {
			...
			await pointer.moveTo(destination.x, destination.y, {
				durationMs: finiteNumber(
					step.durationMs ?? config.dragDurationMs,
					900,
					0,
					5000,
				),
	...
	if (action === "type") {
		...
		if (value === undefined)
			throw new Error(
				`Step "${step.label || index}" has no text or textFromEnv value`,
			);
		if (step.textFromEnv) {
		...
		const target = await preparePointerActivation(
			locator,
			page,
			config,
			pointer,
	...
	if (action === "press") {
		...
		if (scope === "target") {
			...
			if (!focusConfirmed)
				throw new Error(`Step "${step.label || index}" target did not focus`);
		...
		const keyboardCue = await pointer.beginKeyboard(targetBox, keyChord, {
			leadMs: finiteNumber(
				step.keyboardCueLeadMs ?? config.keyboardCueLeadMs,
				320,
				0,
				2000,
			),
			holdMs: finiteNumber(
				step.keyboardCueHoldMs ?? config.keyboardCueHoldMs,
				420,
				0,
				2500,
			),
