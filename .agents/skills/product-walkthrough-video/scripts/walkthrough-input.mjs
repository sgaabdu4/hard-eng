...
function auditStepInputEvidence(step, { pointerEnabled = true } = {}) {
	...
	const events = Array.isArray(step.metadata?.inputEvents)
		? step.metadata.inputEvents
		: [];
	if (events.length !== 1) {
		add(
	...
	if (
		evidence.schemaVersion !== inputEvidenceVersion ||
		evidence.inputType !== expectedType
	) {
		add(
	...
	if (
		...
		add(
			"input-activation-time-invalid",
			"activation time falls outside the recorded action",
		);
	}
	if (
		!Number.isFinite(evidence.settleMs) ||
		evidence.settleMs < minimumSettleMs
	) {
		add(
	...
	if (expectedType === "pointer") {
		...
		const pointerPosition = normalizedPoint(
			evidence.pointerPositionAtActivation,
		);
		if (!targetBox)
			add(
				"input-target-box-invalid",
				"pointer activation has no valid target box",
			);
		if (!pointerPosition) {
			add(
				"input-pointer-position-invalid",
				"pointer activation has no valid pointer position",
			);
		} else if (targetBox && !pointInsideBox(pointerPosition, targetBox)) {
			add(
				"pointer-target-mismatch",
				"pointer activation happened outside the target box",
			);
		}
	...
	if (expectedType === "keyboard") {
		...
		if (evidence.scope === "target") {
			...
			if (!targetBox || !focusBox || !boxesOverlap(targetBox, focusBox)) {
				add(
					...
					"keyboard focus does not match the target box",
				);
			}
			if (evidence.focusConfirmed !== true) {
				add(
					"keyboard-focus-missing",
					"keyboard target was not confirmed as focused",
				);
			}
		}
		...
		if (
			...
			add(
				"keyboard-cue-missing",
				"keyboard activation has no complete visible key cue",
			);
		}
