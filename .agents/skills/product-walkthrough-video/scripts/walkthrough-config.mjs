...
const sensitivePatterns = [
	...
	},
	{
		...
		pattern:
			/(?:token|secret|password|passwd|api[_-]?key)\s*[=:]\s*[^&\s]{8,}/i,
	},
];
...
function includesAllowedSubstring(value, entries) {
	...
	return (
		...
		entries.some((entry) => text.includes(String(entry)))
	);
}
...
function getModifiers(step) {
	...
	if (!Array.isArray(step.modifiers))
		throw new Error(
			`Step "${step.label || step.action}" modifiers must be an array`,
		);
...
function validateConfig(config, baseUrl) {
	...
	const pointerEnabled =
		config.pointer !== false && config.pointer?.enabled !== false;
	...
	if (!readySelector) {
		...
		problems.push(
			"strictE2E requires a product-specific readySelector, not html/body/*",
		);
	}
	...
	for (const [index, step] of config.steps.entries()) {
		...
		if (!String(step.label || "").trim())
			problems.push(`${prefix} needs a descriptive label`);
		...
		if (
			...
			if (!step.target && !step.selector)
				problems.push(
					`${prefix} "${step.label || step.action}" needs a stable target`,
				);
		}
		if (
			strict &&
		...
		if (step.action === "press") {
			...
			if (!String(step.key || "").trim())
				problems.push(`${prefix} press needs a key`);
			...
			if (step.scope !== "global" && !hasTarget) {
				problems.push(
					`${prefix} press needs a stable target or scope "global"`,
				);
			}
			const cueLeadMs = finiteNumber(
				step.keyboardCueLeadMs ?? config.keyboardCueLeadMs,
				320,
			);
			const cueHoldMs = finiteNumber(
		...
		if (step.preserveVisualDuringReload === true && step.action !== "reload") {
			problems.push(
				`${prefix} preserveVisualDuringReload is only valid for reload steps`,
			);
		}
		if (
		...
		if (step.action === "drag") {
			...
			if (!step.to && !hasTargetDescriptor(step.toTarget)) {
				problems.push(
					`${prefix} drag needs a relative "to" point or stable "toTarget"`,
				);
			}
		...
		if (step.action === "scroll" && !step.target && !step.selector) {
			...
			if (strict && durationMs < 600)
				problems.push(
					`${prefix} smooth scroll duration must be at least 600ms`,
				);
		}
	}
	if (strict) {
		...
		if (
			!finalSteps.some((step) =>
				["assertText", "assertUrl", "assertVisible"].includes(step.action),
			)
		) {
			problems.push(
				"strictE2E requires a final assertion within the last four steps",
			);
		}
	}
	return problems;
