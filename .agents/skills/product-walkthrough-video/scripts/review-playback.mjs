...
function videoContentType(videoPath) {
	return path.extname(videoPath).toLowerCase() === ".webm"
		? "video/webm"
		: "video/mp4";
}
	...
	const server = createServer((request, response) => {
		...
		const end =
			requestedStart === null
				...
				: Math.min(
						videoBuffer.length - 1,
						requestedEnd ?? videoBuffer.length - 1,
					);
		if (
	...
	try {
				...
				new Promise((resolve, reject) => {
					...
					element.addEventListener(
						...
						() => {
							...
							reject(
								new Error(
									element.error?.message || "Browser could not load the video",
								),
							);
						},
						{ once: true },
		...
		const state = await video.evaluate((element) => {
			...
			return {
				...
				maxSampleWallGapMs,
				maxMediaWallDriftMs,
				sampledMediaElapsedMs:
					first && last ? (last.mediaTime - first.mediaTime) * 1000 : 0,
				sampledWallElapsedMs:
					first && last ? last.wallTime - first.wallTime : 0,
				inPageElapsedMs:
		...
		const timingToleranceMs = Math.min(
			250,
			Math.max(100, probe.durationMs * 0.0025),
		);
		...
		if (state.seekingEvents > 0)
			throw new Error(
				`Playback seeked ${state.seekingEvents} time(s) after starting`,
			);
		...
		if (state.nonMonotonicSamples > 0)
			throw new Error("Playback media time moved backwards");
		if (state.maxSampleWallGapMs > 500) {
			throw new Error(
				`Playback sampling stalled for ${Math.round(state.maxSampleWallGapMs)}ms`,
			);
		}
		...
		if (
			wallClockElapsedMs < minimumElapsedMs ||
			state.inPageElapsedMs < minimumElapsedMs
		) {
			throw new Error(
		...
		if (
			wallClockElapsedMs > maximumElapsedMs ||
			state.inPageElapsedMs > maximumElapsedMs
		) {
			throw new Error(
		...
		if (
			...
			state.sampledMediaElapsedMs < minimumElapsedMs
		) {
			throw new Error(
		...
		if (pageErrors.length > 0)
			throw new Error(`Playback page error: ${pageErrors.join("; ")}`);
