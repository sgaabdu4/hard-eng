...
async function validateRunReportBinding(review, inputPath) {
	if (
		!review.runReport ||
		!/^[a-f0-9]{64}$/.test(review.runReportSha256 || "")
	) {
		throw new Error(
			"Source review is not bound to a complete journey run report",
		);
	}
	...
	if (sha256(runReportBuffer) !== review.runReportSha256) {
		throw new Error(
			"Source review's journey run report no longer matches its recorded hash",
		);
	}
	...
	if (runReport.sensitiveFindings?.length) {
		throw new Error(
			"Source journey run report contains sensitive-data findings",
		);
	}
	if (!runReport.videoPath || path.resolve(runReport.videoPath) !== inputPath) {
		throw new Error(
			"Source journey run report does not identify the input WebM",
		);
	}
	return runReport;
}
...
async function main() {
	...
	if (output.codec !== "h264")
		throw new Error(`Expected H.264 output, received ${output.codec}`);
