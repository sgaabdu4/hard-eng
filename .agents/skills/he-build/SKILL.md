...
- Reconcile the final diff against every accepted requirement and later correction. Required behavior/proof is complete, confirmed review findings are resolved, and no build blocker remains. Review claims against actual tests/runtime evidence; plan checkboxes alone are declarations.
- Record actual implementation evidence in the plan, check only verified acceptance items, set Status Complete + Verification Passed, then run the existing [Complete check](../he/references/gates.md#plan-checks) on the final integrated code. Failure → follow that gate's Draft/blocker recovery and resume the loop; never announce Ready for ship. Code/configuration changes after a pass require affected proof + the final gate again.
- After the gate passes, record `Ready for ship — local implementation and verification complete; delivery not performed.` in the same plan's Verification section. Handoff = implemented outcome + actual checks/runtime evidence + pending delivery requirements. Authorized delivery continues through [HE Ship](../he-ship/SKILL.md); build alone adds no delivery authority. No BUILD.md or second state record.
