...
- Required stage gates cannot be skipped: Plan context/state validation, Implement owner read/change/guardrails, Verify tests/guardrails, Ship git status/hook readiness/quality gates/no-mistakes, Learn durable-owner/proof
...
- Plan requires passed context and state-validation guardrails; Verify requires the quality gate; Ship requires git status, worktree readiness, quality gate, and a passed `no-mistakes axi run --intent ...` receipt
