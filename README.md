...
`check-no-mistakes-projects.mjs` inventories Git roots when run at a checkout root, excluding vendor and ignored cache, artifact, and build directories. It blocks unconfigured unmanaged nested repos and leaves tracked submodules to their upstreams. At Ship, the root and every configured nested project must have `.no-mistakes.yaml`, an initialized `no-mistakes` remote, active pre-push hooks, and passing project quality gates. `check-project-quality-gates.mjs` fails closed when project-file inventory exceeds seven directory levels or 5,000 files; `check-no-mistakes-projects.mjs` fails closed when nested-repository inventory exceeds seven levels or 500 roots, or when an inventoried directory is unreadable. The default full-repo CI lane relaxes only the remote check with `--allow-missing-no-mistakes-remote`.
