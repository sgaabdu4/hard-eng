...
[![Version](https://img.shields.io/badge/version-0.1.0--alpha.5-f59e0b)](#versioning)
...
Current version: `0.1.0-alpha.5` from [VERSION](VERSION). The matching Git tag is `v0.1.0-alpha.5`.
...
- `approvalBoundaries[]`: real/generated credentials, native permission prompts, prod/backend writes, backend permission/schema/index changes, production payment/email/SMS/sharing side effects, and cleanup approval proof; each entry records `id`, `category`, `status`, `reason`, and `evidence[]`
- `repeatMisses[]`: repeated user-caught issue classes with `issueClass` and `evidence[]` that must route to `/he:learn`
