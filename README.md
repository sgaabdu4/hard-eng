...
- Plan requires passed context and state-validation guardrails; Implement requires ordered `sequence` proof that `test-first` and `test-first-proof` precede `owner-change`, then a passed `find-deterministic-owner.mjs --json` guardrail and green `implementation-proof`; Verify requires the quality gate; Ship requires git status, worktree readiness, quality gate, `no-mistakes axi run --intent ...`, PR evidence repair, `--check-review-threads`, and CI evidence
