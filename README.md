...
- Install pre-push checks for the actual pushed commits, GitHub Actions checks and agent session/failure/completion hooks. Ordinary prompts and successful tools trigger no Hard Eng callback. Remote branch protection requires separate approval.
...
- Configure Context Mode and Codebase Memory; add Appwrite, Sentry, Dart and Marionette MCPs when applicable. Verify relevant integrations before relying on them; startup does not probe every service. Registration alone does not prove readiness.
