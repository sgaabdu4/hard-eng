...
Before changing code, Hard Eng checks the current checkout. It keeps working in an existing linked worktree. A clean primary checkout can continue on its current branch; a primary checkout with staged, unstaged, or untracked changes moves the task into a new isolated worktree so those changes remain untouched. Hard Eng does not require a branch-name prefix.
