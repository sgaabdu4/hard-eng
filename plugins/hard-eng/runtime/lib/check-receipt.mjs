...
export function verifyCheckReceipt(receipt, { key, run, repoId, checkoutId, now = Date.now() } = {}) {
  ...
  if (Date.parse(receipt.expires_at) < now) throw new Error('Check receipt has expired.');
  if (Date.parse(receipt.issued_at) > now + 30_000) throw new Error('Check receipt issue time is invalid.');
