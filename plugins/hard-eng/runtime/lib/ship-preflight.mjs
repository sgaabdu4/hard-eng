...
function changedEntries(repo) {
  ...
  for (let index = 0; index < parts.length;) {
    ...
    if (/^[RC]/.test(status)) entries.push({ status, path: parts[index++] ?? first });
...
export function runShipPreflight(repo, run, { allowedUntracked = [] } = {}) {
  ...
  for (const entry of entries) {
    if (sensitivePath.test(entry.path)) {
      findings.push(finding('sensitive-path', 'critical', 'Environment, credential, key, or secret-bearing paths cannot enter the candidate.', entry.path));
    }
    if (generatedPath.test(entry.path)) {
      findings.push(finding('generated-owner', 'high', 'Generated or changelog output was edited instead of its source owner.', entry.path));
    }
  ...
  const hasSensitivePath = entries.some((entry) => sensitivePath.test(entry.path));
  ...
  const core = {
    ...
    changed_paths_digest: digestValue(entries.map((entry) => ({ status: entry.status, path_digest: sha256(entry.path) }))),
