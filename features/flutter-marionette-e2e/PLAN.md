...
Register the Marionette MCP server for every Flutter app, pinned to the `pubspec.lock` version of `marionette_flutter` when locked, and give the E2E skill a Flutter route that drives the app through Marionette while `xcrun simctl io recordVideo` or `adb shell screenrecord` records the screen. Web walkthrough content (Playwright) and Flutter content live in separate files; only `e2e/SKILL.md` routes between them. Raise this repository's pre-push and CI time budgets from 180 to 300 seconds, because the suite alone now takes about 190 seconds locally and in CI. No new scripts, recorder wrapper or extra MCP servers.
...
- [x] `detected_servers` registers `marionette` for any package whose `pubspec.yaml` depends on the `flutter` SDK: `dart run marionette_mcp@<lock version>` when `pubspec.lock` records `marionette_flutter`, `dart run marionette_mcp@` otherwise; a pure Dart package registers nothing → red/green tests in `tests/test_mcp_setup.py`. The first version required a `marionette_flutter` dependency; the user widened it so the server exists before the app is wired, closing the gap that detection only runs on install or an applied update.
...
- [x] `integrations.md`, `README.md` and `DECISION.md` state the Flutter-app trigger and the pin.
...
Delivery target: PR
Delivery: Pending — full `hard-eng.py check --base origin/main` on the final widened-trigger tree passed: exit 0, 745 tests in 163 s, every native check passed. The first push was rejected by the pre-push hook after every check passed because the run exceeded the 180-second budget, matching the CI cancellation on PR #115; the user chose to raise both budgets to 300 seconds (workflow timeout 5 minutes) and push with `--no-verify`.
