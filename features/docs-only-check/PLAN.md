...
Delivery target: PR
Delivery: Pending — `hard-eng.py check --base origin/main` exit 0: all 17 checks passed, 783 tests and 4 performance checks. A first run failed the 700-line limit on `gate_config.py` and the test file; the helpers moved to their existing owners before the passing run. PR CI remains unverified.
