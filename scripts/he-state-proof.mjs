...
const mutationCommandPattern = /(?:^|\s)(?:(?:npx\s+)?stryker\s+run\b|(?:npm|pnpm|yarn|bun)\s+(?:run\s+)?(?:mutation|mutate|mutants?|stryker)(?::[\w:-]+)?\b|(?:mutmut|infection|pitest)\b|cargo\s+mutants\b|make\s+(?:mutation|mutate|mutants?)\b)/i;
const makeItFailCommandPattern = /(?:^|\s)(?:(?:npm|pnpm|yarn|bun)\s+(?:run\s+)?(?:make[-:]?it[-:]?fail|test[-:]?fail|fail[-:]?test)(?::[\w:-]+)?\b|make\s+(?:make[- ]?it[- ]?fail|test[- ]?fail|fail[- ]?test)\b)/i;
