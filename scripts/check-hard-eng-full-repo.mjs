...
for (let index = 0; index < args.length; index += 1) {
  ...
  if (arg === '--json') json = true;
    ...
    console.log(`Usage: check-hard-eng-full-repo.mjs [--list] [--json] [--include-e2e] [--include-evals] [repo]

Runs the deterministic Hard Eng repository gate.
Default scope excludes real E2E dogfood and model evals.`);
    process.exit(0);
  } else {
...
const commands = [
  ...
  nodeFile('tests/skills/no-mistakes/migration-coverage.test.mjs'),
  nodeFile('tests/skills/no-mistakes/pr-evidence.test.mjs'),
