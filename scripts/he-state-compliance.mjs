...
const nonRiskApprovalEvidencePatterns = [
  ...
  /\b(?:no|not|never|without)(?:\s+\w+){0,4}\s+(?:used|use|using|logged(?:\s+in)?|log\s+in|logging\s+in|login|signed(?:\s+in)?|sign\s+in|signing\s+in|authenticated|authenticate|authenticating)(?:\s+\w+){0,4}\s+(?:saved\s+auth|personal\s+accounts?|real\s+credentials?|real\s+accounts?|real\s+users?|saved\s+accounts?|saved\s+sessions?|prod\s+credentials?|production\s+credentials?|prod\s+accounts?|production\s+accounts?|prod\s+sessions?|production\s+sessions?)\b/,
