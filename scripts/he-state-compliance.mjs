...
const approvalBoundaryEvidencePatterns = new Map([
  ['prod-backend-write', [
    ...
    /\b(?:inserted|insert|inserting|upserted|upsert|upserting|patched|patch|patching|uploaded|upload|uploading)\b.*\b(?:prod|production)\b.*\b(?:backend|appwrite|database|db|data|record|records|file|files|user|users|account|accounts|credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets)\b/,
    /\b(?:inserted|insert|inserting|upserted|upsert|upserting|patched|patch|patching|uploaded|upload|uploading)\b.*\b(?:backend|appwrite|database|db|data|record|records|file|files|user|users|account|accounts|credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets)\b.*\b(?:prod|production)\b/,
    /\b(?:prod|production)\b.*\b(?:backend|appwrite|database|db|data|record|records|file|files|user|users|account|accounts|credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets)\b.*\b(?:inserted|insert|inserting|upserted|upsert|upserting|patched|patch|patching|uploaded|upload|uploading)\b/,
    /\b(?:updated|update|updating|modified|modify|modifying|granted|grant|granting|revoked|revoke|revoking)\b.*\b(?:prod|production)\b.*\b(?:backend|appwrite|database|db|permission|permissions|schema|index|email|emails|sms|text|texts|message|messages|payment|payments|charge|charges|refund|refunds|receipt|receipts|card|cards|customer|customers|subscription|subscriptions|invoice|invoices|data|record|records|file|files|link|links|notification|notifications|invite|invites|invitation|invitations|webhook|webhooks|user|users|account|accounts|access|credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets)\b/,
    /\b(?:updated|update|updating|modified|modify|modifying|granted|grant|granting|revoked|revoke|revoking)\b.*\b(?:backend|appwrite|database|db|permission|permissions|schema|index|email|emails|sms|text|texts|message|messages|payment|payments|charge|charges|refund|refunds|receipt|receipts|card|cards|customer|customers|subscription|subscriptions|invoice|invoices|data|record|records|file|files|link|links|notification|notifications|invite|invites|invitation|invitations|webhook|webhooks|user|users|account|accounts|access|credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets)\b.*\b(?:prod|production)\b/,
    /\b(?:prod|production)\b.*\b(?:backend|appwrite|database|db|permission|permissions|schema|index|email|emails|sms|text|texts|message|messages|payment|payments|charge|charges|refund|refunds|receipt|receipts|card|cards|customer|customers|subscription|subscriptions|invoice|invoices|data|record|records|file|files|link|links|notification|notifications|invite|invites|invitation|invitations|webhook|webhooks|user|users|account|accounts|access|credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets)\b.*\b(?:updated|update|updating|modified|modify|modifying|granted|grant|granting|revoked|revoke|revoking)\b/,
    /\b(?:changed|change|changing|wrote|write|writing|mutated|mutation|mutate|deleted|delete|deleting|created|create|creating|disabled|disable|disabling|enabled|enable|enabling|suspended|suspend|suspending|deactivated|deactivate|deactivating|removed|remove|removing|reset|resetting)\b.*\b(?:prod|production)\b.*\b(?:backend|appwrite|database|db|permission|permissions|schema|index|email|emails|sms|text|texts|message|messages|payment|payments|charge|charges|refund|refunds|receipt|receipts|card|cards|customer|customers|subscription|subscriptions|invoice|invoices|data|record|records|file|files|link|links|notification|notifications|invite|invites|invitation|invitations|webhook|webhooks|user|users|account|accounts|credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets)\b/,
    /\b(?:changed|change|changing|wrote|write|writing|mutated|mutation|mutate|deleted|delete|deleting|created|create|creating|disabled|disable|disabling|enabled|enable|enabling|suspended|suspend|suspending|deactivated|deactivate|deactivating|removed|remove|removing|reset|resetting)\b.*\b(?:backend|appwrite|database|db|permission|permissions|schema|index|email|emails|sms|text|texts|message|messages|payment|payments|charge|charges|refund|refunds|receipt|receipts|card|cards|customer|customers|subscription|subscriptions|invoice|invoices|data|record|records|file|files|link|links|notification|notifications|invite|invites|invitation|invitations|webhook|webhooks|user|users|account|accounts|credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets)\b.*\b(?:prod|production)\b/,
    /\b(?:prod|production)\b.*\b(?:backend|appwrite|database|db|permission|permissions|schema|index|email|emails|sms|text|texts|message|messages|payment|payments|charge|charges|refund|refunds|receipt|receipts|card|cards|customer|customers|subscription|subscriptions|invoice|invoices|data|record|records|file|files|link|links|notification|notifications|invite|invites|invitation|invitations|webhook|webhooks|user|users|account|accounts|credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets)\b.*\b(?:changed|change|changing|wrote|write|writing|mutated|mutation|mutate|deleted|delete|deleting|created|create|creating|disabled|disable|disabling|enabled|enable|enabling|suspended|suspend|suspending|deactivated|deactivate|deactivating|removed|remove|removing|reset|resetting)\b/,
  ...
  ['real-credentials', [
    ...
    /\b(?:prod|production)\s+(?:account|accounts|session|sessions|credential|credentials)\b.*\b(?:e2e|smoke|test|tests|spec|playwright)\b.*\b(?:pass|passed|completed|ran|run|succeeded|success)\b/,
  ]],
  ['generated-credentials', [
...
const approvalBoundarySideEffectPatterns = new Map([
  ['prod-backend-write', [
    ...
    ['prod-credential', [
      /\b(?:changed|change|changing|updated|update|updating|modified|modify|modifying|created|create|creating|deleted|delete|deleting|removed|remove|removing|revoked|revoke|revoking|granted|grant|granting|reset|resetting|inserted|insert|inserting|upserted|upsert|upserting|patched|patch|patching)\b.*\b(?:credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets)\b/,
      /\b(?:credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets)\b.*\b(?:changed|change|changing|updated|update|updating|modified|modify|modifying|created|create|creating|deleted|delete|deleting|removed|remove|removing|revoked|revoke|revoking|granted|grant|granting|reset|resetting|inserted|insert|inserting|upserted|upsert|upserting|patched|patch|patching)\b/,
    ]],
    ['prod-sms', [
...
const nonRiskApprovalEvidencePatterns = [
  ...
  /\b(?:no|not|never|without)(?:\s+\w+){0,4}\s+(?:disabled|disable|disabling|enabled|enable|enabling|suspended|suspend|suspending|deactivated|deactivate|deactivating|removed|remove|removing|reset|resetting)(?:\s+\w+){0,4}\s+(?:prod|production)\s+(?:user|users|account|accounts|access)\b/,
  /\b(?:no|not|never|without)(?:\s+\w+){0,3}\s+(?:prod|production)\s+(?:cleanup|write|writes|mutation|delete|backend|appwrite|database|db)\b/,
  /\b(?:no|not|never|without)(?:\s+\w+){0,4}\s+(?:prod|production|backend|appwrite|database|db|permission|permissions|schema|index|indexes|indices|migration|migrations|native|real|personal|saved|generated|test|e2e|account|accounts|credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets|cleanup|email|emails|sms|text|texts|message|messages|payment|payments|charge|charges|refund|refunds|receipt|receipts|card|cards|customer|customers|subscription|subscriptions|invoice|invoices|sharing|shared|data|record|records|file|files|link|links|notification|notifications|invite|invites|invitation|invitations|webhook|webhooks|user|users|access|side effects?)(?:\s+\w+){0,6}\s+(?:write|writes|wrote|writing|mutation|mutate|mutated|mutating|change|changed|changing|update|updated|updating|modify|modified|modifying|insert|inserted|inserting|upsert|upserted|upserting|patch|patched|patching|upload|uploaded|uploading|apply|applied|applying|execute|executed|executing|grant|granted|granting|revoke|revoked|revoking|delete|deleted|deleting|created|create|creating|used|use|using|clicked|click|allow|cleanup|sent|send|sending|emailed|emailing|texted|texting|messaged|messaging|delivered|deliver|delivering|triggered|trigger|triggering|posted|post|posting|called|call|calling|invoked|invoke|invoking|fired|fire|firing|charged|charge|charging|refunded|refund|refunding|shared|share|sharing|published|publish|publishing|notified|notify|notifying|invited|invite|inviting)\b/,
  /\b(?:no|not|never|without)(?:\s+\w+){0,4}\s+(?:write|writes|wrote|writing|mutation|mutate|mutated|mutating|change|changed|changing|update|updated|updating|modify|modified|modifying|insert|inserted|inserting|upsert|upserted|upserting|patch|patched|patching|upload|uploaded|uploading|apply|applied|applying|execute|executed|executing|grant|granted|granting|revoke|revoked|revoking|delete|deleted|deleting|created|create|creating|used|use|using|clicked|click|allow|cleanup|sent|send|sending|emailed|emailing|texted|texting|messaged|messaging|delivered|deliver|delivering|triggered|trigger|triggering|posted|post|posting|called|call|calling|invoked|invoke|invoking|fired|fire|firing|charged|charge|charging|refunded|refund|refunding|shared|share|sharing|published|publish|publishing|notified|notify|notifying|invited|invite|inviting)(?:\s+\w+){0,6}\s+(?:prod|production|backend|appwrite|database|db|permission|permissions|schema|index|indexes|indices|migration|migrations|native|real|personal|saved|generated|test|e2e|account|accounts|credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets|cleanup|email|emails|sms|text|texts|message|messages|payment|payments|charge|charges|refund|refunds|receipt|receipts|card|cards|customer|customers|subscription|subscriptions|invoice|invoices|sharing|shared|data|record|records|file|files|link|links|notification|notifications|invite|invites|invitation|invitations|webhook|webhooks|user|users|access|side effects?)\b/,
];
...
const postposedNonRiskApprovalEvidencePatterns = [
  ...
  /\b(?:prod|production)\b(?:\s+\w+){0,6}\s+(?:backend|appwrite|database|db|permission|permissions|schema|index|email|emails|sms|text|texts|message|messages|payment|payments|charge|charges|refund|refunds|receipt|receipts|card|cards|customer|customers|subscription|subscriptions|invoice|invoices|data|record|records|file|files|link|links|notification|notifications|invite|invites|invitation|invitations|webhook|webhooks|user|users|account|accounts|access|credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets|cleanup|side effects?)\b(?:\s+\w+){0,4}\s+(?:not|never)\s+(?:changed|updated|modified|inserted|upserted|patched|uploaded|applied|executed|wrote|written|mutated|deleted|created|granted|revoked|used|clicked|accepted|allowed|sent|emailed|texted|messaged|delivered|triggered|posted|called|invoked|fired|charged|refunded|shared|published|notified|invited|shown|displayed|performed|run|ran)\b/,
  /\b(?:prod|production)\b(?:\s+\w+){0,6}\s+(?:backend|appwrite|database|db|permission|permissions|schema|index|email|emails|sms|text|texts|message|messages|payment|payments|charge|charges|refund|refunds|receipt|receipts|card|cards|customer|customers|subscription|subscriptions|invoice|invoices|data|record|records|file|files|link|links|notification|notifications|invite|invites|invitation|invitations|webhook|webhooks|user|users|account|accounts|access|credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets|cleanup|side effects?)\b(?:\s+\w+){0,4}\s+(?:did|does|do)\s+not\s+(?:change|update|modify|insert|upsert|patch|upload|apply|execute|write|mutate|delete|create|grant|revoke|use|click|accept|allow|send|email|text|message|deliver|trigger|post|call|invoke|fire|charge|refund|share|publish|notify|invite|show|display|perform|run)\b/,
  /\b(?:backend|appwrite|database|db)?(?:\s+\w+){0,3}\bmigrations?\b(?:\s+\w+){0,6}\s+(?:not|never)\s+(?:applied|apply|applying|ran|run|running|executed|execute|executing)\b(?:\s+\w+){0,8}\s+(?:prod|production)\b/,
  /\b(?:backend|appwrite|database|db)?(?:\s+\w+){0,3}\bmigrations?\b(?:\s+\w+){0,6}\s+(?:prod|production)\b(?:\s+\w+){0,6}\s+(?:not|never)\s+(?:applied|apply|applying|ran|run|running|executed|execute|executing)\b/,
  ...
  /\b(?:email|emails|sms|text|texts|message|messages|payment|payments|charge|charges|refund|refunds|receipt|receipts|card|cards|customer|customers|subscription|subscriptions|invoice|invoices|data|record|records|file|files|link|links|notification|notifications|invite|invites|invitation|invitations|webhook|webhooks|user|users|account|accounts|access)\b(?:\s+\w+){0,6}\s+(?:prod|production)\b(?:\s+\w+){0,4}\s+(?:did|does|do)\s+not\s+(?:send|email|text|message|deliver|trigger|post|call|invoke|fire|charge|refund|share|publish|notify|invite|upload|insert|upsert|patch|delete|create|grant|revoke|disable|enable|suspend|deactivate|remove|reset)\b/,
...
const codeOnlyNativePermissionApprovalEvidencePatterns = [
  ...
  /\b(?:native|permission|prompt|dialog)\b.*\b(?:validator|validation|scanner|test|tests|spec|script|regex|pattern|check|guardrail|gate|code|docs|documentation|metadata|result|results)\b.*\b(?:triggered|trigger|triggering|shown|show|showing|displayed|display|displaying|opened|open|opening|clicked|click|accepted|accept|allowed|allow|granted|grant)\b/,
  /\b(?:validator|validation|scanner|test|tests|spec|script|regex|pattern|check|guardrail|gate|code|docs|documentation|metadata|result|results)\b.*\b(?:native|permission|prompt|dialog)\b.*\b(?:triggered|trigger|triggering|shown|show|showing|displayed|display|displaying|opened|open|opening|clicked|click|accepted|accept|allowed|allow|granted|grant)\b/,
  /\b(?:triggered|trigger|triggering|shown|show|showing|displayed|display|displaying|opened|open|opening|clicked|click|accepted|accept|allowed|allow|granted|grant)\b.*\b(?:native|permission|prompt|dialog)\b.*\b(?:validator|validation|scanner|test|tests|spec|script|regex|pattern|check|guardrail|gate|code|docs|documentation|metadata|result|results)\b/,
];

const codeOnlyCredentialApprovalEvidencePatterns = [
  /\b(?:real|generated|personal|saved|prod|production|test|e2e|credential|credentials|account|accounts|user|users|password|passwords|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets|cleanup)\b.*\b(?:validator|validation|scanner|script|regex|pattern|check|guardrail|gate|code|docs|documentation)\b.*\b(?:changed|change|changing|updated|update|updating|added|add|adding|implemented|implementing|fixed|fixing|repair|repaired|failed|failure|failing|errored|error|errors|passed|pass|passing|result|results|metadata|reported)\b/,
  /\b(?:validator|validation|scanner|script|regex|pattern|check|guardrail|gate|code|docs|documentation)\b.*\b(?:real|generated|personal|saved|prod|production|test|e2e|credential|credentials|account|accounts|user|users|password|passwords|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets|cleanup)\b.*\b(?:changed|change|changing|updated|update|updating|added|add|adding|implemented|implementing|fixed|fixing|repair|repaired|failed|failure|failing|errored|error|errors|passed|pass|passing|result|results|metadata|reported)\b/,
  /\b(?:changed|change|changing|updated|update|updating|added|add|adding|implemented|implementing|fixed|fixing|repair|repaired|failed|failure|failing|errored|error|errors|passed|pass|passing|result|results|metadata|reported)\b.*\b(?:real|generated|personal|saved|prod|production|test|e2e|credential|credentials|account|accounts|user|users|password|passwords|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets|cleanup)\b.*\b(?:validator|validation|scanner|script|regex|pattern|check|guardrail|gate|code|docs|documentation)\b/,
  /\b(?:real|generated|personal|saved|prod|production|test|e2e|credential|credentials|account|accounts|user|users|password|passwords|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets|cleanup)\b.*\b(?:unit\s+test|mock|mocked|stub|stubbed|fixture|spec\s+only|test\s+double|test|tests|spec)\b.*\b(?:changed|change|changing|updated|update|updating|added|add|adding|implemented|implementing|fixed|fixing|repair|repaired|metadata|reported)\b/,
  /\b(?:unit\s+test|mock|mocked|stub|stubbed|fixture|spec\s+only|test\s+double|test|tests|spec)\b.*\b(?:real|generated|personal|saved|prod|production|test|e2e|credential|credentials|account|accounts|user|users|password|passwords|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets|cleanup)\b.*\b(?:changed|change|changing|updated|update|updating|added|add|adding|implemented|implementing|fixed|fixing|repair|repaired|metadata|reported)\b/,
...
const codeOnlyProdCleanupApprovalEvidencePatterns = [
  /\b(?:prod|production)\b.*\bcleanup\b.*\b(?:validator|validation|scanner|regex|pattern|guardrail|gate|check|checks|code|docs|documentation|unit\s+test|mock|mocked|stub|stubbed|fixture|spec\s+only|test\s+double|test|tests|spec)\b.*\b(?:changed|change|changing|updated|update|updating|added|add|adding|implemented|implementing|fixed|fixing|repair|repaired|failed|failure|failing|errored|error|errors|pass|passed|passing|result|results|metadata|reported)\b/,
  /\bcleanup\b.*\b(?:prod|production)\b.*\b(?:validator|validation|scanner|regex|pattern|guardrail|gate|check|checks|code|docs|documentation|unit\s+test|mock|mocked|stub|stubbed|fixture|spec\s+only|test\s+double|test|tests|spec)\b.*\b(?:changed|change|changing|updated|update|updating|added|add|adding|implemented|implementing|fixed|fixing|repair|repaired|failed|failure|failing|errored|error|errors|pass|passed|passing|result|results|metadata|reported)\b/,
  /\b(?:validator|validation|scanner|regex|pattern|guardrail|gate|check|checks|code|docs|documentation|unit\s+test|mock|mocked|stub|stubbed|fixture|spec\s+only|test\s+double|test|tests|spec)\b.*\b(?:prod|production)\b.*\bcleanup\b.*\b(?:changed|change|changing|updated|update|updating|added|add|adding|implemented|implementing|fixed|fixing|repair|repaired|failed|failure|failing|errored|error|errors|pass|passed|passing|result|results|metadata|reported)\b/,
  /\b(?:changed|change|changing|updated|update|updating|added|add|adding|implemented|implementing|fixed|fixing|repair|repaired|failed|failure|failing|errored|error|errors|pass|passed|passing|result|results|metadata|reported)\b.*\b(?:prod|production)\b.*\bcleanup\b.*\b(?:validator|validation|scanner|regex|pattern|guardrail|gate|check|checks|code|docs|documentation|unit\s+test|mock|mocked|stub|stubbed|fixture|spec\s+only|test\s+double|test|tests|spec)\b/,
];
...
const hypotheticalApprovalEvidencePatterns = [
  /\b(?:skipped|skip|not run|preflight|dry run|hypothetical|simulated|simulation)\b(?:\s+\w+){0,10}\s+(?:would|could|might|may|avoid|avoids|avoided|avoiding)\b(?:\s+\w+){0,10}\s+(?:open|opened|opening|show|shown|showing|display|displayed|displaying|trigger|triggered|triggering)\b(?:\s+\w+){0,8}\s+(?:native|permission|prompt|dialog)\b/,
  /\b(?:would|could|might|may)\s+(?:be\s+)?(?:open|opened|opening|show|shown|showing|display|displayed|displaying|trigger|triggered|triggering)\b(?:\s+\w+){0,8}\s+(?:native|permission|prompt|dialog)\b/,
  /\b(?:open|opening|show|showing|display|displaying|trigger|triggering)\b(?:\s+\w+){0,8}\s+(?:native|permission|prompt|dialog)\b(?:\s+\w+){0,6}\s+(?:would|could|might|may)\b/,
  /\b(?:avoid|avoids|avoided|avoiding)\b(?:\s+\w+){0,8}\s+(?:open|opened|opening|show|shown|showing|display|displayed|displaying|trigger|triggered|triggering)\b(?:\s+\w+){0,8}\s+(?:native|permission|prompt|dialog)\b/,
  /\b(?:skipped|skip|not run|preflight|dry run|hypothetical|simulated|simulation)\b(?:\s+\w+){0,10}\s+(?:would|could|might|may|avoid|avoids|avoided|avoiding)\b(?:\s+\w+){0,10}\s+(?:insert|inserted|inserting|upsert|upserted|upserting|patch|patched|patching|upload|uploaded|uploading|apply|applied|applying|run|ran|running|execute|executed|executing|deliver|delivered|delivering|trigger|triggered|triggering|post|posted|posting|call|called|calling|invoke|invoked|invoking|fire|fired|firing)\b(?:\s+\w+){0,8}\s+(?:prod|production|backend|appwrite|database|db|native|real|generated|credential|credentials|cleanup|email|emails|sms|text|texts|message|messages|payment|payments|card|cards|customer|customers|data|record|records|file|files|webhook|webhooks|migration|migrations|user|users|account|accounts|permission|permissions|schema|index|access)\b/,
...
const generatedCredentialCleanupMisbindingPattern = /\b(?:deleted|removed|purged|revoked)\b(?:\s+\w+){0,5}\s+(?:ticket|issue|request|proof|evidence|note|artifact|case|audit\s+logs?|logs?|events?|reports?|metadata|entries|(?:cleanup\s+)?record)\b/i;
const generatedCredentialCleanupTargetedPositivePatterns = [
  /\b(?:cleaned up|cleaned-up|deleted|removed|purged|revoked)\b(?:\s+\w+){0,5}\s+(?:generated|seeded|redacted|credential|credentials|account|accounts|user|users|password|passwords|identity|session)\b/i,
  /\b(?:generated|seeded|redacted|credential|credentials|account|accounts|user|users|password|passwords|identity|session)\b(?:\s+\w+){0,5}\s+(?:cleaned up|cleaned-up|deleted|removed|purged|revoked)\b/i,
  /\b(?:source of truth|cleanup target|cleanup result)(?:\s+\w+){0,8}\s+(?:cleaned up|cleaned-up|deleted|removed|purged|revoked|confirmed|verified|complete|completed|done)\b/i,
  /\b(?:generated|seeded|redacted|credential|credentials|account|accounts|user|users|password|passwords|identity|session)\b(?:\s+\w+){0,5}\s+cleanup(?:\s+\w+){0,5}\s+(?:confirmed|verified|complete|completed|done|success|succeeded)\b/i,
  /\b(?:cleanup)\b(?:\s+\w+){0,5}\s+(?:complete|completed|done|confirmed|verified|success|succeeded)(?:\s+\w+){0,5}\s+(?:generated|seeded|redacted|credential|credentials|account|accounts|user|users|password|passwords|identity|session|source of truth|cleanup target|cleanup result)\b/i,
  /\b(?:confirmed|verified|complete|completed|done|success|succeeded)\b(?:\s+\w+){0,5}\s+(?:cleanup|delete|deleted|remove|removed|purge|purged)(?:\s+\w+){0,5}\s+(?:generated|seeded|redacted|credential|credentials|account|accounts|user|users|password|passwords|identity|session|source of truth|cleanup target|cleanup result)\b/i,
  /\b(?:test|e2e)\s+(?:user|users|account|accounts|credential|credentials|password|passwords)(?:\s+\w+){0,5}\s+(?:cleaned up|cleaned-up|deleted|removed|purged|revoked)\b/i,
  /\b(?:cleaned up|cleaned-up|deleted|removed|purged|revoked)\b(?:\s+\w+){0,5}\s+(?:test|e2e)\s+(?:user|users|account|accounts|credential|credentials|password|passwords)\b/i,
];
...
function hasPositiveGeneratedCredentialCleanupProof(cleanupProof) {
  ...
  return proofSegments.some((segment) => (
    ...
    && !generatedCredentialCleanupMisbindingPattern.test(segment)
    && matchesAny(segment, generatedCredentialCleanupTargetedPositivePatterns)
  ));
}
...
function sideEffectMentionMatches(category, sideEffectKey, text) {
  ...
  const mentionPatterns = new Map([
    ...
    ['prod-credential', [/\b(?:prod|production)\b.*\b(?:credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets)\b/, /\b(?:credential|credentials|api\s+key|api\s+keys|key|keys|token|tokens|secret|secrets)\b.*\b(?:prod|production)\b/]],
...
function approvalBoundaryRequirementsForText(text) {
  ...
  for (const normalized of segments) {
    for (const subsegment of approvalRiskCandidateSubsegments(normalized)) {
      ...
      if (matchesAny(subsegment, approvalObjectBeforeVerbRiskPatterns)) matchedCategories.add('prod-backend-write');
