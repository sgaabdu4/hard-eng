...
const explicitApprovalGrantPattern = /\b(?:approved|authorized|authorised|okayed|signed off)\b|\b(?:approval|authorization|authorisation|consent)\b(?:\s+\w+){0,3}\s+granted\b|\bgranted(?:\s+\w+){0,3}\s+(?:approval|authorization|authorisation|consent)\b|\b(?:user|operator|maintainer|owner|requester|human|manual|explicit)\b(?:\s+\w+){0,5}\s+(?:confirmed|allowed|permission(?:\s+\w+){0,3}\s+granted|granted(?:\s+\w+){0,3}\s+permission)\b|\b(?:confirmed|allowed|permission(?:\s+\w+){0,3}\s+granted|granted(?:\s+\w+){0,3}\s+permission)\b(?:\s+\w+){0,5}\s+(?:by|from)\s+(?:user|operator|maintainer|owner|requester|human)\b|\b(?:approval|authorization|authorisation|consent)\b(?:\s+\w+){0,3}\s+(?:confirmed|allowed)\b|\b(?:confirmed|allowed)\b(?:\s+\w+){0,3}\s+(?:approval|authorization|authorisation|consent)\b/i;
