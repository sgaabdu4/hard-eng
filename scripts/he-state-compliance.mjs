...
const postposedNonRiskApprovalEvidencePatterns = [
  ...
  /\b(?:prod|production)\b(?:\s+\w+){0,6}\s+(?:backend|appwrite|database|db|permission|permissions|schema|index|email|emails|sms|text|texts|message|messages|payment|payments|charge|charges|refund|refunds|receipt|receipts|card|cards|customer|customers|subscription|subscriptions|invoice|invoices|data|record|records|file|files|link|links|notification|notifications|invite|invites|invitation|invitations|webhook|webhooks|user|users|account|accounts|access|cleanup|side effects?)\b(?:\s+\w+){0,4}\s+(?:not|never)\s+(?:changed|updated|modified|wrote|written|mutated|deleted|created|granted|revoked|used|clicked|accepted|allowed|sent|emailed|texted|messaged|delivered|triggered|posted|charged|refunded|shared|published|notified|invited|shown|displayed|performed|run)\b/,
  /\b(?:email|emails|sms|text|texts|message|messages|receipt|receipts)\b(?:\s+\w+){0,4}\s+(?:prod|production)\b(?:\s+\w+){0,4}\s+(?:not|never)\s+(?:sent|emailed|texted|messaged|delivered|triggered|posted|notified|invited)\b/,
