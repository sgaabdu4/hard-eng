...
assertIncludes(readmeText, 'Same-repo owner PRs that only update `vendor/skill-upstreams/<name>` gitlinks use deterministic vendor-integrity and CI proof instead; matching `VERSION` and README alpha-version bumps may be omitted, but if included they must pass the version contract.');
...
assertIncludes(noMistakesRequiredText, 'isMaintainerSubmoduleOnlyUpdate');
assertIncludes(noMistakesRequiredText, 'isSkillSubmoduleGitlinkUpdate');
assertIncludes(noMistakesRequiredText, 'isReadmeVersionBump');
