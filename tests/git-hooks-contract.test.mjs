...
assert.ok(installScript.includes('git -C "$repo" cat-file -e "$remote_sha^{commit}"'), 'pre-push history scan must guard unavailable remote commits');
assert.ok(installScript.includes('pushed_revs+=("${remote_sha}..${local_sha}")'), 'pre-push history scan must scan updated ref ranges');
assert.ok(installScript.includes('pushed_revs+=("$local_sha")'), 'pre-push history scan must fall back to local commit scans');
...
if (fs.existsSync(prePushHook)) {
  ...
  assert.ok(text.includes('git -C "$repo" cat-file -e "$remote_sha^{commit}"'), 'installed pre-push hook must guard unavailable remote commits');
