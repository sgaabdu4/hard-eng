...
const keyDefinitions = [
  ...
  'requiresProjectAgentsTokenCap: applies the global repo-specific cap of 600 o200k tokens to project AGENTS.md while still rejecting duplicated local Budget sections, scripts, hooks, or gates',
  ...
  'usesUiReviewReceipt: uses a saved ui-review-receipt from the real or fallback review surface when UI flow or visual design needs user choice',
  'requiresUiReviewReceipt: requires durable receipt evidence before accepting a UI decision',
  'requiresSavedUiChoicesComponents: requires saving selected choice, rejected options, and chosen components for UI decisions',
  'requiresPlanUiScreenshots: requires he-plan UI review receipts to include screenshotPaths covering every shown UI/design option and userVisibleEvidence proving screenshots or visual artifacts were shown before acceptance',
  'requiresImplementationUiScreenshots: requires UI-touched he-implement work to capture actual implementation screenshots from the real route, app surface, Storybook, Widgetbook, simulator, or localhost and record a passed implementation-ui-screenshots guardrail before /he:verify',
  'blocksLateUiDecisionDiscovery: blocks discovering basic UI design or flow choices only during implement/verify/ship when he-plan lacked visible screenshots, visual artifacts, or user acceptance',
  'usesUiReviewReceiptForNonUi: incorrectly uses UI review receipts for generic plans, reports, non-UI diagrams, or stage management',
  ...
  'classifiesDirtyScope: requires dirty worktree state to be classified, including feature/unclassified files, untracked artifacts, and vendor or submodule-only dirties, before ship loop-complete',
  'usesArtifactHygieneScanner: requires a deterministic artifact hygiene scanner such as check-hard-eng-artifacts.mjs for raw logs, emails, secrets, sessions, UUIDs, large payloads, or untracked proof artifacts before PR/ship',
  'usesWriteSafetyScanner: requires a deterministic write-safety scanner such as check-hard-eng-write-safety.mjs for scripts that mutate Appwrite, GitHub, curl, fetch, or backend APIs, checking dry-run default, explicit write flag, allowlist, approval boundary, and post-write proof',
  ...
  'allowsMarkedLargeOwnerOver700: allows marked hook/scanner/parser/contract/eval owners with focused behavior tests to exceed 700 lines',
