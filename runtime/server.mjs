...
function applyRunEvent(store, verified, args, now, supportObserver) {
  ...
  if (current.cursor.step === 'await-user-clarification' && event?.type !== 'clarification.answered') {
    throw new Error('Run is in a clarification hold; only the exact user answer may be recorded.');
  }
  ...
  if (['clarification.required', 'clarification.answered'].includes(event?.type)) {
    const currentCandidate = fingerprintCandidate(verified.checkout_root, { allowAllUntracked: true });
    event = {
      ...event,
      plan_digest: current.plan?.digest ?? null,
  ...
  if (event?.type === 'external-action.prepared' && event.action?.intent === 'publish') {
    ...
    const head = git(verified.checkout_root, ['rev-parse', 'HEAD']).trim();
    const parents = git(verified.checkout_root, ['show', '-s', '--format=%P', commit]).trim().split(/\s+/).filter(Boolean);
    const working = fingerprintCandidate(verified.checkout_root, { allowAllUntracked: true });
    if (
      commit !== publication?.commit
      || head !== commit
      || parents.length !== 1
      || parents[0] !== current.candidate?.head
      || fingerprintCommitTree(verified.checkout_root, commit) !== current.candidate?.tree_fingerprint
      || working.tree_fingerprint !== current.candidate?.tree_fingerprint
    ) throw new Error('Publication preparation requires the exact clean local commit over the approved candidate.');
    const commitMessage = validateCommitMessage(git(
    ...
    const publicationPreflight = observePublicationPreparation(
      ...
      current.candidate,
      {
        preparedCommit: {
          commit,
          commit_message_digest: commitMessage.message_digest,
          commit_message_evidence_digest: commitMessage.evidence_digest,
        },
      },
    );
    event = {
  ...
  function verifyLocalPublication(publication) {
    ...
    const preflight = publication?.preflight ?? current.interruption?.publication_preflight;
    const commitMessage = validateCommitMessage(git(
      verified.checkout_root,
      ['show', '-s', '--format=%B', commit],
    ));
    if (
      ...
      || preflight.commit_message_digest !== commitMessage.message_digest
      || preflight.commit_message_evidence_digest !== commitMessage.evidence_digest
    ) throw new Error('Published commit message differs from its validated pre-publication evidence.');
