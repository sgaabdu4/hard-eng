...
const cursorSteps = {
  Plan: new Set(['discover', 'prototype', 'ready-for-approval', 'await-user-clarification']),
  Build: new Set(['red', 'implement', 'verify', 'review', 'slice-proven', 'await-user-review', 'learn', 'await-user-clarification']),
  Ship: new Set(['preflight', 'await-candidate-approval', 'publish', 'await-publication-approval', 'learn', 'await-user-clarification']),
...
function validateCursor(run, nested = false) {
  const cursor = run.cursor;
  let allowed = new Set(['step']);
  if (cursor.step === 'await-user-clarification') {
    if (nested) throw new Error('Clarification holds cannot contain another clarification hold.');
    allowed = new Set([
      'step', 'reason_code', 'question_ids', 'conflict_digest', 'plan_digest',
      'candidate_fingerprint', 'source_revision', 'return_boundary',
    ]);
  } else if (run.phase === 'Build') {
  ...
  if (cursor.step === 'await-user-clarification') {
    if (!/^[a-z0-9][a-z0-9-]{1,63}$/.test(cursor.reason_code ?? '')) throw new Error('Clarification reason code is invalid.');
    if (
      !Array.isArray(cursor.question_ids)
      || cursor.question_ids.length < 1
      || cursor.question_ids.length > 3
      || new Set(cursor.question_ids).size !== cursor.question_ids.length
      || cursor.question_ids.some((id) => !/^[a-z0-9][a-z0-9._-]{1,63}$/.test(id))
    ) throw new Error('Clarification question IDs are invalid.');
    assertDigest(cursor.conflict_digest, 'Clarification conflict digest');
    assertDigest(cursor.candidate_fingerprint, 'Clarification candidate fingerprint');
    if (cursor.plan_digest !== null) assertDigest(cursor.plan_digest, 'Clarification plan digest');
    if (!Number.isInteger(cursor.source_revision) || cursor.source_revision < 1) throw new Error('Clarification source revision is invalid.');
    const boundary = cursor.return_boundary;
    if (
      !isPlainObject(boundary)
      || !['Plan', 'Build', 'Ship'].includes(boundary.phase)
      || !isPlainObject(boundary.cursor)
      || !cursorSteps[boundary.phase].has(boundary.cursor.step)
    ) throw new Error('Clarification return boundary is invalid.');
    assertAllowedKeys(boundary, new Set(['phase', 'cursor']), 'Clarification return boundary');
    validateCursor({ phase: boundary.phase, cursor: boundary.cursor }, true);
    return;
  }
...
export function validateSession(session) {
  ...
  if (!Number.isInteger(session.binding_revision) || session.binding_revision < 1) {
    throw new Error('Session binding revision is invalid.');
  }
  if (typeof session.revoked !== 'boolean') throw new Error('Session revocation state is invalid.');
  if (session.pending !== null) {
    if (!isPlainObject(session.pending)) throw new Error('Session pending action is invalid.');
    assertAllowedKeys(session.pending, new Set(['key', 'action', 'started_at']), 'Session pending action');
    assertDigest(session.pending.key, 'Session pending action key');
    if (!['start', 'resume', 'event'].includes(session.pending.action)) {
      throw new Error('Session pending action kind is invalid.');
    }
    if (!validTimestamp(session.pending.started_at)) throw new Error('Session pending action timestamp is invalid.');
    if (session.revoked) throw new Error('A revoked session cannot retain a pending action.');
  }
  ...
  for (const replay of session.replays) {
    if (!isPlainObject(replay)) throw new Error('Session replay entry is invalid.');
    assertAllowedKeys(replay, new Set(['key', 'result']), 'Session replay entry');
    assertDigest(replay.key, 'Session replay key');
    if (replayKeys.has(replay.key)) throw new Error('Session replay keys must be unique.');
    replayKeys.add(replay.key);
    const result = replay.result;
    if (!isPlainObject(result)) throw new Error('Session replay result is invalid.');
    assertAllowedKeys(result, new Set([
      'status', 'run_id', 'phase', 'cursor', 'intent', 'findings', 'next',
      'revision', 'capsule', 'interruption',
    ...
    if (
      ...
      || !cursorSteps[result.phase]?.has(result.cursor.step)
    ) throw new Error('Session replay result binding is invalid.');
    validateCursor({ phase: result.phase, cursor: result.cursor });
    if (
      !isPlainObject(result.intent)
      || !['plan', 'direct'].includes(result.intent.kind)
      || Object.keys(result.intent).sort().join(',') !== 'digest,kind'
    ) throw new Error('Session replay intent is invalid.');
    assertDigest(result.intent.digest, 'Session replay intent digest');
    if (
      !isPlainObject(result.findings)
      || Object.keys(result.findings).join(',') !== 'open'
      || !Number.isInteger(result.findings.open)
      || result.findings.open < 0
    ) throw new Error('Session replay finding summary is invalid.');
    if (!isPlainObject(result.next)) throw new Error('Session replay next action is invalid.');
    assertAllowedKeys(result.next, new Set(['owner', 'action']), 'Session replay next action');
    const expectedNext = nextFor(result.phase, result.cursor);
    if (result.next.owner !== expectedNext.owner || result.next.action !== expectedNext.action) {
      throw new Error('Session replay next action does not match its cursor.');
    }
    if (!Number.isInteger(result.revision) || result.revision < 1) throw new Error('Session replay revision is invalid.');
    if (typeof result.capsule !== 'string' || !result.capsule || result.capsule.length > 4_096) {
      throw new Error('Session replay capsule is invalid.');
    }
    if (result.interruption !== undefined && !isPlainObject(result.interruption)) {
      throw new Error('Session replay interruption is invalid.');
    }
  }
  if (session.replays.length > 0 && session.run_id === null) throw new Error('Unbound sessions cannot retain replay results.');
  if (!validTimestamp(session.updated_at)) throw new Error('Session update timestamp is invalid.');
