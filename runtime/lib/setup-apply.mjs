...
function copyBackup(source, target, type) {
  ...
  if (type === 'directory') {
    copyDirectoryExact(source, target);
    return;
  }
...
function persistRollbackBundle({
  ...
  try {
    ...
    const receipt = {
      ...
      previous_codex_mcp_status: plan.codex_mcp?.before_status ?? 'NOT_CONFIGURED',
    ...
    if (moved && fs.existsSync(destination)) fs.rmSync(destination, { recursive: true, force: true });
    throw error;
  }
}
...
function applyOperation({ home, sourceRoot, operation, transaction, transactionContext, index }) {
  ...
  const backup = current && current.type !== 'symlink'
    ? path.join(transaction, `${index}.backup`)
    : null;
  ...
  if (onRollback) {
    try {
      ...
      throw new AggregateError(
        [error, recoveryError],
        'Setup failed and external Codex wiring reconciliation also failed.',
      );
    }
  }
...
export function applySetupPlan(plan, {
  ...
  const previousRecord = readInstallManifestRecord(home);
  if ((previousRecord?.hash ?? null) !== plan.existing_manifest_hash) {
    throw new Error('Install manifest changed after setup approval.');
  }
  const previousManifest = previousRecord?.bytes ?? null;
  const applied = [];
  ...
  try {
    ...
    const finalized = finalize ? finalize({ transaction, transactionContext }) : null;
    const finalization = finalized?.result ?? finalized;
    if (finalized?.applied !== undefined) {
      if (!Array.isArray(finalized.applied)) throw new Error('Setup finalization applied ledger is invalid.');
      applied.push(...finalized.applied);
    }
    transactionContext.mark('external-applied');
