...
from .prepare import prepare, remove_copy, remove_shared, share, status
...
--repo gives the Git repository its own copy of the newest Hard Eng and stages the bootstrap, hooks, and rules
so every clone fetches it too.
"""
...
def _command_update(arguments: argparse.Namespace) -> int:
    state = share(Path(arguments.repo or os.getcwd()), _home(arguments.home), arguments.agent)
    _print_state(state, json_output=arguments.json)
    return 0
...
def _command_uninstall(arguments: argparse.Namespace) -> int:
    ...
    if inspect_repository(root).shared:
        remove_shared(root)
        print(f"removed the Hard Eng wiring from {root}; commit the deletions to remove it from every clone")
    elif remove_copy(root):
        print(f"removed the stale Hard Eng copy from {root}")
    else:
        print("Hard Eng is not installed in this repository")
    return 0
...
def _command_install(arguments: argparse.Namespace) -> int:
    ...
    return install_repository(Path(os.getcwd()), home)
