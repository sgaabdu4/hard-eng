...
def old_generation(root: Path, changes: dict[str, str]) -> list[str]:
    ...
    from agent_hooks import CLAUDE_IMPORT, HOOK_FILES
    ...
    for name, header in OLD_GENERATION.items():
        ...
        used = name in wiring or (
            name.startswith(".hard-eng/")
            and (".hard-eng" in wiring or Path(name).name in wiring)
        )
        if text[-1].startswith(header) and not used:
            retired.append(name)
    ...
    removable = retire_tracked(root, retired)
