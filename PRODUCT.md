...
- `hard-eng start <codex|claude|copilot>` = tracked `hard-eng.gates.json` admission + healthy-global selection OR GitHub-attested immutable release under ignored `.agents/hard-eng/` + repository-owned `AGENTS.md` bridge + native skills/hooks/MCP wiring before agent startup; no marker = exact pass-through; partial global state = fail closed; tracked repository bytes remain unchanged.
