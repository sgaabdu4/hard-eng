...
| Gates | `deterministic-checks` | migrates missing project gate wiring before product mutation → proves compatible real-tool seams before paid/native retries → runs affected-full gates with manifest-bound argv, mandatory declared boundary contracts, direct Zod 4 plus lockfile proof for marked TypeScript/React projects, and full zero-finding latest quality analyzers; independent read-only families run in bounded parallel workers while exclusive scanners stay protected |
