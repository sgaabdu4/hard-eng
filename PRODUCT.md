...
| Repository startup | `bin/hard-eng` + `runtime/repository_native/` | unmarked repository passes through unchanged → marked repository uses one healthy global Hard Eng OR one verified ignored fallback → partial global state + unsafe provider conflicts stop before the agent → each marked start rechecks global health or the allowed immutable release before Codex, Claude Code, or Copilot CLI runs |
