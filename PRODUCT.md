...
|---|---|---|
| Lifecycle | `he` `he-plan` `he-build` `he-ship` `he-learn` | route/state selected → feature setup green → one lean Feature Brief approved before full-gate debt → complete end-to-end behavior first → slices converge → green artifact delivered → proven process gap prevented |
