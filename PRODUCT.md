...
|---|---|---|
| Lifecycle | `he` `he-plan` `he-build` `he-ship` `he-learn` | route/state selected → feature setup green → one lean Feature Brief approved before full-gate debt → complete end-to-end behavior first → slices converge sequentially or as parallel ticket worktrees behind one integration gate → green artifact delivered → proven process gap prevented |
...
| Repository startup | `bin/hard-eng` + `runtime/repository_native/` | unmarked repository passes through unchanged → marked repository uses one healthy global Hard Eng OR one verified ignored fallback → partial global state + unsafe provider conflicts stop before the agent → fallback Copilot prompt mode requires explicit repository-hook trust → each marked start rechecks global health or the allowed immutable release before Codex, Claude Code, or Copilot CLI runs |
...
- `hard-eng start <codex|claude|copilot>` = tracked `hard-eng.gates.json` admission + healthy-global selection OR GitHub-attested immutable release under ignored `.agents/hard-eng/` + repository-owned `AGENTS.md` bridge + native skills/hooks/MCP wiring + verified in-session `hard-eng status` fallback before agent startup; fallback Copilot `-p`/`--prompt` fails closed without explicit `--trust-repository-hooks`, which enables repository hooks only for that launched process; no marker = exact pass-through; partial global state = fail closed; tracked repository bytes remain unchanged.
...
- Execution evidence = current `research.json` + exact `authorization.json`; configured Direct work records one Git-private request/session receipt whose frozen paths + exact named connector action digests/effects gate live known writes while the later checkpoint rechecks repository and research drift; unavailable identity transport uses that bound receipt, but any supplied mismatch blocks; irreversible destructive actions keep the separate one-use exact `protected-action.json`.
