...
|---|---|---|
| Lifecycle | `he` `he-plan` `he-build` `he-ship` `he-learn` | route/state selected → feature setup green (base branch resolved from `origin/main|develop`, `feature/<slug>` worktree created, ignored env inputs listed and copied after one batched question) → one lean Feature Brief approved before full-gate debt, and only after six machine-checked planning receipts (code study, research, edge scan, decision inventory, slice graph, closing ticket question) → ready-for-handoff block printed with checkout, branch, plan path, and a paste-ready build prompt → complete end-to-end behavior first → every slice leaves four machine-checked records (edge cases mapped to success and failure tests, the green run, a fresh-eyes review ledger of at most three rounds, an independent verification with faked outside services and before/after evidence) that the slice gate refuses to skip → slices converge sequentially or as at most four parallel ticket worktrees behind one integration gate, with the orchestrator blocked from editing claimed ticket paths → mutation scores kept in a committed ledger, filled nightly on public repositories and before push on private ones, with every survivor needing a verdict → one closing question (walkthrough video?) → generated `BUILD.md` + ship handoff block → green artifact delivered → proven process gap prevented |
...
| Gates | `deterministic-checks` | migrates missing project gate wiring before product mutation → at setup preserves any configured JS/TS formatter or linter silently, or chooses Biome only when neither exists → proves compatible real-tool seams before paid/native retries → runs the one manifest-owned commit, push, or CI phase → checks staged files at commit and runs affected-full gates before delivery → keeps local and CI command sets identical → runs independent read-only families in bounded parallel workers while exclusive scanners stay protected → Fallow runs as a new-only audit (JSON report, baselined) and jscpd fails only on clone pairs absent from the committed baseline |
| Repository setup | `bin/hard-eng` + `runtime/repository_native/` | unmarked repository passes through unchanged → marked repository uses one healthy global Hard Eng OR its own repository copy → partial global state + unsafe provider conflicts stop before any write → repository copy = composed `AGENTS.override.md` for Codex + `CLAUDE.local.md` import for Claude Code + `.github/instructions/hard-eng.instructions.md` for Copilot + native hooks, skills, agents, and output-style links → generated files self-heal on rerun and refuse hand edits → a later healthy global removes the stale copy → shared wiring commits one marker + bootstrap + guard shim + hook entries + rule surfaces so every fresh clone fetches the newest Hard Eng `main` at session start, refreshes it whenever `main` moves, keeps the copy when offline, denies tool calls until a first fetch succeeds, and defers to a healthy global guard on developer machines |
| Terminal setup | private `package.json` bin → `install.sh` → `runtime/repository_native/installer.py` | `npx -y github:sgaabdu4/hard-eng --global` clones the newest Hard Eng `main` into `~/.agents`, fast-forwards a clean existing clone, replaces an old release install, or repairs a modified development checkout in place → per-agent ready/skipped report → `npx -y github:sgaabdu4/hard-eng --repo` runs only at a Git root + creates and stages missing repository rules and the shared marker + prepares Codex, Claude, and Copilot together under one lock with journaled rollback → stages the committed bootstrap, shim, hook entries, and rule surfaces → machine-specific wiring is always privately ignored + existing tracked files remain unchanged except the shared generated set |
...
- `npx -y github:sgaabdu4/hard-eng --global|--repo` = one GitHub entry through the private package's only bin; global mode clones the newest Hard Eng `main` at `~/.agents`, fast-forwards a clean clone, replaces an old release install by staging + swap + `setup.sh install` + rollback on failure, or repairs a modified development checkout, then reports every agent as ready or skipped (CLI not installed); repository mode runs at the exact Git root + creates missing `AGENTS.md`, `CLAUDE.md`, and `hard-eng.gates.json` + stages the marker and the generated files every clone needs + always privately ignores machine-specific wiring + prepares Codex, Claude, and Copilot together; one flock lock per target + journaled rollback leave no partial state behind.
...
- `hard-eng prepare|status|update|uninstall` = tracked or privately ignored `hard-eng.gates.json` admission + healthy-global selection OR the repository's own shallow clone of Hard Eng `main` under ignored `.agents/hard-eng/` + generated `AGENTS.override.md` (repository rules first, Hard Eng rules second, raises Codex's own project document limit to 64 KiB in one owned `.codex/config.toml` line and fails closed above it) + `CLAUDE.local.md` import + `.github/instructions/hard-eng.instructions.md` + native hooks/skills/agents/output-style links + `generated` digests that heal untouched drift and refuse hand edits + uninstall that strips only Hard Eng hook entries and its `.codex/config.toml` line from user-modified files; no marker = exact pass-through; marker keys other than `schema_version` + `wiring` = refused; partial global state = fail closed; tracked repository bytes remain unchanged; the guard answers Copilot in its own hook format when Copilot invokes the Claude hook entry. + native hook trust: Codex runs repository hooks only in trusted projects after its hook review (`codex exec` needs `--dangerously-bypass-hook-trust` until reviewed), Copilot runs repository hooks only inside `trustedFolders` while the global user-level hook runs everywhere, Claude Code runs them in every mode including `-p` + shared mode: the committed `.hard-eng/bootstrap.sh` compares the local copy with `main` on GitHub at every session start and swaps in a fresh shallow clone when `main` moved (one lock, atomic symlink swap, older copies pruned, offline keeps the copy); `.hard-eng/hook.sh` runs the cached guard, exits 0 when `.agents/hard-eng/global-guard` lists the agent, and otherwise fetches Hard Eng via the committed bootstrap before a pre-tool call and denies only when no copy exists; committed hook files merge Hard Eng session-start + pre-tool entries with git-root command paths and keep foreign entries; tracked generated files are admitted only from the shared set and healed only when they carry the Hard Eng marker; `uninstall` removes the generated files, strips Hard Eng hook entries, and drops the marker
