#!/usr/bin/env python3
"""Initialize, discover, validate, and freshness-check Hard Eng PLAN.md state."""
...
def inspect(repo_arg: str, plan_arg: str | None) -> int:
    ...
    for raw_path in raw_paths:
        try:
            ...
            records.append((path, load_plan(path)))
        except (OSError, UnicodeError, PlanStateError) as exc:
    ...
    if len(active) > 1:
        ...
        for index, (path, state) in enumerate(active, start=1):
            emit(
                f"candidate_{index}",
