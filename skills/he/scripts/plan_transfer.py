...
def file_snapshot(path: Path, label: str) -> tuple[bytes, int]:
...
def exact_applied(path: Path, expected: tuple[bytes, int]) -> bool:
    try:
        return file_snapshot(path, str(path)) == expected
    except PlanStateError:
        return False


def restore(path: Path, original: tuple[bytes, int] | None) -> None:
    if original is None:
...
def transfer_plan(
    ...
    *,
    ...
    write: Callable[[Path, bytes, int], None] = atomic_write,
) -> int:
    ...
    def rollback() -> list[str]:
        errors: list[str] = []
        if source_written and source_plan is not None and source_original is not None:
            try:
                atomic_write(source_plan, *source_original)
            except OSError as error:
                errors.append(f"source:{error}")
        for path, original in reversed(tuple(destination_originals.items())):
            try:
        ...
        for directory in reversed(created_directories):
            try:
                ...
                pass
        if normal_transfer and common_dir is not None and pending_manifest(common_dir):
            try:
    ...
    try:
        ...
        with repository_transaction(common_dir, rollback):
            ...
            source_original = file_snapshot(source_plan, "source PLAN")
            ...
            for raw in includes:
                ...
                content, mode = file_snapshot(source_path, f"include {relative}")
                destination_path = exact_path(destination_root, str(relative))[1]
                status = path_status(destination_root, relative)
                if status and not (manifest is not None and exact_applied(destination_path, (content, mode))):
            ...
            if destination_plan_status and not (
                manifest is not None and exact_applied(destination_plan, plan_expected)
            ):
                raise PlanStateError("destination PLAN path is dirty or colliding")

            source_document_token = document_token(source_text)
            if document_token(source_plan.read_text(encoding="utf-8")) != source_document_token:
            ...
            for relative, (destination_path, content, mode) in included.items():
                source_path = exact_path(source_root, str(relative))[1]
                if file_snapshot(source_path, f"include {relative}") != (content, mode):
                    raise PlanStateError(f"source include changed during transfer: {relative}")
                destination_originals[destination_path] = (
                    file_snapshot(destination_path, f"destination {relative}")
                    if destination_path.exists()
                    else None
                )
            destination_originals[destination_plan] = (
                file_snapshot(destination_plan, "destination PLAN") if destination_plan.exists() else None
            )
            ...
            targets = [*(entry[0] for entry in included.values()), destination_plan]
            ...
            if normal_transfer:
                ...
                source_written = True
            for destination_path, content, mode in included.values():
                if not exact_applied(destination_path, (content, mode)):
                    write(destination_path, content, mode)
            if not exact_applied(destination_plan, plan_expected):
                write(destination_plan, *plan_expected)

            destination_result = validate_document(
                destination_plan, destination_plan.read_text(encoding="utf-8")
            )
            if freshness_errors(
            ...
            source_result = validate_document(source_plan, source_plan.read_text(encoding="utf-8"))
