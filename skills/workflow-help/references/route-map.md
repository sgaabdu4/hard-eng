...
- Record `planReadiness.uiReview.receipt` only for UI option decisions: surface kind, review target, artifact path, receipt path, exact question/options, saved choices, saved components, selected option, rejected options, chosen components, and user decision
