...
| 2. Implement | Readiness is `PASS` and code changes are needed. | Use `he-implement`. Require `entryGate.fromStage = he-plan` and `decision = PASS`. Load `test-quality`, list behavior scenarios, add or identify the smallest failing test first, record the red state as `test-first-proof`, then change the canonical owner. If red-first is impossible, run mutation/"make it fail" proof before readying Verify. Run `find-deterministic-owner.mjs --json` and record `deterministic-owner-scan` before fresh reasoning. Repeat work runs its deterministic owner first. Every violation gets lint/scanner/gate (script/test/hook/eval); duplicate-prone values or concepts also get SSOT scanner/registry coverage. Add or wire deterministic guardrails in `guardrails[]`; React/Next changes need React Doctor + Fallow audit/dupes + lint/typecheck gate, Flutter changes need package-root `dart analyze` with `flutter_skill_lints` plus tests. Use `codebase-design` when owner/abstraction is unclear. Add needed skills and learning findings for repeated misses or future guards. | Receipt with root owner change, TDD proof, guardrail owner, proof to run, learning findings or no-learning evidence, and `Next: ready for /he:verify: yes/no`. |
...
- `next.ready: true` is invalid without the stage-required guardrails: context/state validation for Plan, TDD `test-first-proof` and touched-stack inventory for Implement, quality gate for Verify, and git status/worktree-ready/quality gate/no-mistakes for Ship
