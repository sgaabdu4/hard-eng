...
- Ship ready requires positive ordered `sequence` evidence for git status, worktree readiness, format check, project inventory, and quality gate before the latest `no-mistakes`; PR evidence, review-thread proof, and CI follow in that order. Named Ship guardrails use reachable, failure-propagating executable invocations, not comments, quoted/echoed text, shell/runtime indirection, masked/piped/background commands, post-exit commands, or unreachable branches
