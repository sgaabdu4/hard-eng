...
- Before every visible Grill Me question, classify the premise and each option as proven, unresolved, or unsupported. Ask only unresolved choices, apply proven facts silently, discard unsupported options, and mark user-confirmed only from an exact user answer or accepted UI review receipt. Persist the exact visible question block; if state says no question, emit none
- Record `planReadiness.sourceCoverage` for every plan. When sources exist, bind each project-local path to its `git-blob:<object-id>` revision, SHA-256 digest, line counts, and disjoint nonblank-line spans whose statuses and references map completely to the one accepted canonical `plan.md` plus separate project-local evidence. Missing or changed sources, gaps, overlaps, open/contradictory items, and missing or invalid references block Plan `PASS` and later ready handoffs. When no source exists, record `required: false`, a reason, and concrete evidence in the accepted plan artifact; omission is not readiness
- Record `planReadiness.uiReview.receipt` only for UI option decisions: accepted status, `surfaceKind` (`real-route`, `react-localhost`, `storybook`, `flutter-widget-preview`, `widgetbook`, `simulator`, or `local-html`), project-local artifact/receipt/saved-choice/saved-component paths, exact question/options, selected/rejected options, chosen components, structurally valid PNG/JPEG/WebP screenshot paths for every option, user-visible evidence that every screenshot was shown before acceptance, evidence, and user decision. Presentation uses `user-opened-review-surface` and a distinct project-local `ui-presentation/v1` Browser/Chrome/Computer Use event binding the surface and screenshot digests, question, presentation time, and later approval; the event is consistency evidence, while the workflow must obtain the actual user-opened presentation and later approval. Browser surfaces need localhost `surfaceUrl`; simulator needs `deviceTarget`; Flutter Widget Previewer/Widgetbook need localhost `surfaceUrl` or `deviceTarget`
...
- `next.ready: true` is invalid without the stage-required guardrails: context/state validation for Plan, ordered TDD `test-first-proof` with explicit `test-quality` evidence, green `implementation-proof`, and touched-stack inventory for Implement, quality gate for Verify, git status/worktree-ready/format-check/project-inventory/quality gate/no-mistakes for Ship, and `ship-currentness` for Ship loop-complete; complete or evidenced-not-required source coverage is also mandatory
- Ship ready requires positive ordered `sequence` evidence for git status, worktree readiness, format check, project inventory, and quality gate before the latest `no-mistakes`; PR evidence, review-thread proof, and CI follow in that order. Named Ship guardrails use direct, reachable, failure-propagating executable invocations in the recorded repo context, not comments, quoted/echoed text, shell/runtime indirection, shell control flow, execution-context mutation, arbitrary inline environment assignments other than literal `NODE_ENV=test`, masked/piped/background commands, post-exit commands, or unreachable branches
