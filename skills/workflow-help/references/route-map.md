...
- Record `planReadiness.uiReview.receipt` only for UI option decisions: accepted status, surface kind, surface URL or device target when required, artifact path, receipt path, saved choices/components paths, exact question/options, selected/rejected options, chosen components, evidence, and user decision
