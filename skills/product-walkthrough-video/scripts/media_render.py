...
def promote_output(temporary: Path, destination: Path, step: str) -> None:
    require(
        temporary.is_file() and temporary.stat().st_size > 0,
        step,
        "local media command produced no output",
    )
    require(
        not destination.exists(),
        step,
...
def render_scene(
    ...
    if visual["kind"] == "image":
        ...
        argv.extend(
            [
                "-ss",
                f"{visual['trim_start_seconds']:.6f}",
                "-i",
                str(visual["path"]),
            ]
        )
    argv.extend(
        [
            ...
            f"[1:a]apad=pad_dur={context['render']['tail_seconds']:.6f}[a]",
            "-map",
            "[v]",
            "-map",
            "[a]",
...
def render(context: dict[str, Any]) -> None:
    ...
    run_checked(
        [str(context["ffmpeg"]), "-hide_banner", "-h", "filter=silenceremove"],
        ...
        scenes.append(
            {
                ...
                "duration_seconds": media_duration(
                    context, scene_path, f"render.scene.{chapter['id']}.probe"
                ),
                "trimmed_audio": {
    ...
    concat_payload = "".join(
        f"file '{item['path'].replace(chr(39), chr(92) + chr(39))}'\n"
        for item in scenes
    )
...
def qa(context: dict[str, Any]) -> None:
    ...
    require(
        final.is_file()
        and render_receipt.get("artifact", {}).get("sha256") == digest(final),
        step,
    ...
    violations = [
        ...
        and (
            ...
            or any(
                event["start"] <= boundary <= event["end"] for boundary in boundaries
            )
        )
    ]
    require(
        not violations,
        "qa.silence-boundaries",
        "scene boundary silence exceeds the configured maximum",
    )
    ...
    try:
        run_checked(
            [
                ...
                "-vf",
                f"fps=1/{interval},"
                f"scale={context['render']['width'] // columns}:-2,"
