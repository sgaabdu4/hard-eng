...
10. **Bulk before per-row staging** — same mutation across matching rows uses server-SDK `updateRows`/`deleteRows`; heterogeneous create/upsert uses `createRows`/`upsertRows`. One bulk request is atomic; one bulk call staged with `transactionId` consumes one transaction operation while its rows stay subject to the separate bulk request cap. Over-cap work uses deployment-bound, ID-scoped chunks + durable checkpoints + exact fixed-point read-back. Use [bulk-operations](references/bulk-operations.md).
