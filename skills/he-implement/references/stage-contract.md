...
- For any UI-touched work, capture actual implementation screenshots from the real route/surface after `implementation-proof` and before `/he:verify`; this is required real UI proof and cannot be replaced by unit tests, curl, or command-only evidence. Record it as a passed `implementation-ui-screenshots` guardrail with screenshot paths in evidence and an ordered `sequence` after `owner-change` and `implementation-proof`
