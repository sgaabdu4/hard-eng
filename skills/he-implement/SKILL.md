...
- For React/Next, wire React Doctor, Fallow duplicate/clone evidence (`fallow dupes` or audit evidence naming clone groups), lint, and positive typecheck pass/result evidence into a deterministic script or pre-push hook. For Flutter, wire package-root `dart analyze` with `flutter_skill_lints` and tests when present
