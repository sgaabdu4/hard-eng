...
Read `../workflow-help/references/route-map.md`, `../test-quality/SKILL.md`, and `references/ssot-owner-reuse.md` before acting, then load touched-area skills only.
...
- Before `test-first` or any owner change, complete `ssot-owner-reuse`: prove shared/component/pattern/domain owners, duplicate/clone evidence, and reuse/extend/create decisions; UI/component work cannot skip SSOT scanner evidence with `not_applicable`
...
- For any UI-touched work, capture actual implementation screenshots from the real route/surface after `implementation-proof` and before `/he:verify`; record them as a passed `implementation-ui-screenshots` guardrail with screenshot paths in evidence and an ordered `sequence` after `owner-change` and `implementation-proof`
...
- Record touched-stack guardrail coverage in `guardrailInventory.touchedStacks[]` and `guardrailInventory.requiredGuardrails[]`: regex scanners, Git hooks, lint/analyze/typecheck, SSOT scanners, Fallow, React Doctor, and repeat-mistake prevention are `required` with a matching `guardrails[]` entry or `not_applicable` with reason/evidence
- For React/Next, wire React Doctor, Fallow duplicate/clone result evidence (`fallow dupes` or audit output for clean or found clone groups), lint, and positive typecheck pass/result evidence into a deterministic script or pre-push hook. For Flutter, wire package-root `dart analyze` with `flutter_skill_lints` and tests when present
...
- Exit with the stage receipt: state path, decision, owner/proof, artifacts, blocker, `SSOT reused`, `SSOT extended`, `new owners created`, `Next: ready for /he:verify: yes/no`, and `Handover prompt:` for a fresh session with worktree, `he-state.json`, blockers, artifacts, and the next `/he:verify` command. Only `PASS` can say ready yes. No transcript dump
