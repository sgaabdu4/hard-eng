...
UI_EXT = {".tsx", ".jsx", ".dart"}
TEST_PATH_MARKERS = ("__tests__/", ".test.", ".spec.", "_test.dart", "/test/")
...
def media_error(paths: tuple[str, ...], e2e_value: str) -> str | None:
    if not e2e_value.startswith("not-applicable:"):
        return None
    if not any(ui_path(path) for path in paths):
        return None
    return "this slice changes UI files: provide an actual-media e2e receipt instead of --e2e not-applicable"
