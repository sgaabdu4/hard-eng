...
- Families = `targeted | typecheck format lint tests fallow | react-doctor | dart-analyze dart-test dart-decimate | boundary-contracts`; every applicable family needs one project-manifest command with exit `0`.
- Boundary contracts = a `families.boundary-contracts` entry in `hard-eng.gates.json` marks the project; relevant JS/TS/React, Dart/Flutter, and contract/config changes must include that family, and missing coverage fails closed. Marked TypeScript/React projects must declare direct `zod@4`, have exactly one recognized lockfile resolving Zod 4, and run the project-owned Zod boundary command.
