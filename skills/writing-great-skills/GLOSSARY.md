...
A skill that keeps implicit routing enabled, so the agent can see its **description** and fire it autonomously. The human can still type its name unless menu visibility is disabled. Pays a permanent **context load** on every turn in exchange for that discoverability. Reachable by other skills, because the description that makes it agent-discoverable makes it invocable. A model-invoked skill whose content is all **reference** is also one home for shared reference: another skill can invoke it, so reference needed by several skills lives in one place. Pick model-invocation only when the agent must reach the skill on its own; if it never fires except by hand, disable implicit routing and pay no model-routing load.

_Avoid_: ability, tool, capability

### User-Invoked

A skill whose implicit routing is disabled — hidden from the agent's autonomous choice and reachable only by the human typing its name (user-_only_, where **model-invoked** is user-_and-agent_). Trades agent-discoverability for zero model-routing load. In cross-agent skills, this means `disable-model-invocation: true` for Claude-style runtimes and Codex `agents/openai.yaml` with `policy.allow_implicit_invocation: false`. Because the model cannot select it, no other skill should depend on firing it implicitly.

_Avoid_: procedure, workflow, command

### Description

The skill's machine-readable trigger, and the one **context pointer** a **model-invoked** skill is forced to keep loaded at all times. Its presence plus enabled implicit routing is the invocation axis: keep both and the skill is model-invoked; disable implicit routing and the skill is **user-invoked**, reachable only by the human. The source of a model-invoked skill's **context load**.
...
A **user-invoked** skill whose job is to point at your other user-invoked skills — naming each and when to reach for it — so the human has one skill to remember instead of many. It can only hint, never fire them: user-invoked skills are removed from implicit routing, so nothing but the human can reach them. The cure for **cognitive load** when user-invoked skills multiply.
