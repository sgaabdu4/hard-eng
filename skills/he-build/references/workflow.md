...
7. Prior axes PASS/N/A + `review=pending` → checkpoint → bounded exact-evidence shards + rules/context + secret gate → `owner-first` + `boundary-first` per shard → parent root ledger → full-coverage convergence pass. New root → update ledger + one confirmation pass; no new root → bounded stability PASS; new root on final allowed round → `AUDIT_INVENTORY_UNSTABLE` + shared-owner pause before product mutation. Stability ≠ mathematical exhaustiveness.
