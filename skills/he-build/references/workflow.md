...
7. Prior axes PASS/N/A + `review=pending` → checkpoint → bounded exact-evidence shards + rules/context + secret gate → `owner-first` + `boundary-first` per shard → parent root ledger → full-coverage convergence pass. New root → update ledger + one confirmation pass; no new root → bounded stability PASS. Final-round new root → lossless FAIL/CONCERNS aggregate + `inventoryStable=false`; every root → PLAN evidence; unchanged-snapshot replay = forbidden. Stability ≠ mathematical exhaustiveness.
8. Coverage = every primary changed path assigned once/pass; dependency overflow → continuation shards + exact context. One warm probe → completion-driven fan-out ≤8 from latency + deadline + `--latency-profile ordinary|urgent`; convergence reuses bounded capacity + deadline/cancellation. Aggregate capacity = actual review-unit count × strict limits → aggregate validated findings/unknowns losslessly by semantic root; indivisible primary evidence overflow + truncation/omission = fail closed. `cached_input_tokens` = telemetry only; child profile denies source/controller homes.
