...
7. Prior axes PASS/N/A + `review=pending` → checkpoint → bounded exact-evidence shards + rules/context + secret gate → two independent inventory passes/shard (`owner-first` + `boundary-first`) → every primary path × applicable lens + continue after first root → one warm probe → completion-driven fan-out ≤8 from observed latency + deadline + selected `--latency-profile ordinary|urgent`; `cached_input_tokens` = telemetry only; child profile denies source/controller homes.
8. Coverage = every primary changed path assigned once/pass; dependency overflow → deterministic continuation shards + exact context coverage → aggregate capacity = review unit count × strict per-unit limits before reviewer launch → aggregate validated findings/unknowns by semantic root losslessly into one verdict; indivisible primary evidence overflow = exact fail-closed owner; truncation/omission = forbidden.
