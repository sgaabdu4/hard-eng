...
def check_inventory_convergence_regressions(module, fail, snapshot: str) -> None:
    ...
    reviewed, combined, rounds, executed_batches = module.converge_inventory(
    ...
    try:
        ...
        if "AUDIT_INVENTORY_UNSTABLE" not in str(error):
            fail("unstable inventory lost its structured shared-owner boundary")
    else:
        fail("audit allowed product mutation while new roots still appeared")
