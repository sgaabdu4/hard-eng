...
- For UI work, use Grill Me UI flow/visual design with `atomic-ui`, `impeccable`, and Impeccable Live; show a localhost mock flow using the current design system, fonts, and shared components. Use Lavish only to compare UI options and collect decisions through `npx -y lavish-axi poll`; save the poll receipt, selected choice, rejected options, and chosen components before approval
