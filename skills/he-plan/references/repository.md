...
3. Apply `$deterministic-checks` worktree owner → select checkout + classify ignored state/setup/smoke owners → require `write` PASS.
...
Complete = accepted checkout `write` PASS + ignored/setup/smoke proof + every identity field proven + root pair valid/current/approved; repository stage cannot skip.
