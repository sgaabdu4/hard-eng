...
1. Read repository evidence + canonical owners + inspect the relevant current product screen + affected flow → current truth known → `record-step code-study` with `answers` (could break + owner + existing capability + external contract); visual reference → `atomic-ui` verifies root `DESIGN.md` + production owners; current screenshots ground the proposed state but never substitute for it.
...
4. Resolve material uncertainty via `question-me` dependency frontiers + batch independent decisions + no per-section approval → `record-step decisions` after every answer (each decision carries `rejected`); record its not-yet-specifiable items as `deferred` and its user-action items as `blocked_on`, then continue.
5. Enumerate every vertical slice with `depends_on` → `record-step slices` with `answers` (`thinnest_path` + `parallel`); run `plan_state.py validate` → deterministic PASS with `plan_steps=5/6`.
...
8. Standard mode → show the complete brief + the closing question (split into tickets? which probed tracker?) in one message → user's plain yes (any literal reply) → `record-step closing` with the ticket answer + `unknowns` + pass the same reply to `plan_state.py approve --approval-reply` → show the printed handoff block → `he-build`; decision answers + pre-brief replies = reject.
