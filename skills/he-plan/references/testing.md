...
2. Use `$test-quality` → obtain named behavior/risk proof design per row; record its layer + pass criterion + every required production/proof/config owner as repeated same-slice `` `owner:S-#:repository/relative/path` `` edges.
