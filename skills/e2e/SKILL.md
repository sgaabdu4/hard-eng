...
- Real credentials, generated users/passwords, native permission prompts, prod/backend writes, backend permission/schema/index/migration/webhook changes, production payment/email/SMS/sharing side effects, and prod cleanup are approval boundaries; record approval, redacted scope, and cleanup proof in `he-state.json`
