...
- Signed-in browser state, cookies, extensions, or existing user profile: use the available Chrome or Browser control capability
...
If Browser control or the `browser-client` path is missing, record that exact missing capability in the run report and continue to Playwright or Computer Use.
