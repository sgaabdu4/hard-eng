...
A bad ref or empty diff fails before any review pass.
...
Run the Standards and Spec passes directly by default. If the user explicitly
requests delegation, the two passes may use independent subagents in parallel;
the parent still verifies every finding against the source evidence.
...
Keep the reports separate in either mode.
