...
When evidence permits more than one safe interpretation, show the exact
conflict. For a bound run, checkpoint `clarification.required`, ask the targeted
question, and wait without changing source or candidate state. Resume the exact
recorded boundary only through `clarification.answered`; never infer an answer
from silence, history, or a new task.
